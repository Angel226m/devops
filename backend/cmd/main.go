...
func main() {
	...
	// ✅ Ejecutar migraciones (iniciales + incrementales)
	log.Println("📋 Ejecutando migraciones...")
	if err := runMigrations(db); err != nil {
		log.Fatalf("❌ Error al ejecutar migraciones: %v", err)
	}
...
// runMigrations ejecuta migraciones iniciales e incrementales
func runMigrations(db *sql.DB) error {
	// 1️⃣ Crear tabla de control de migraciones si no existe
	if err := createMigrationsTable(db); err != nil {
		return fmt.Errorf("error al crear tabla de migraciones:  %v", err)
	}

	// 2️⃣ Ejecutar migración inicial (crear_tablas.sql) si es necesario
	if err := runInitialMigration(db); err != nil {
		return fmt.Errorf("error en migración inicial: %v", err)
	}

	// 3️⃣ Ejecutar migraciones incrementales (ampliar columnas, etc.)
	if err := runIncrementalMigrations(db); err != nil {
		return fmt.Errorf("error en migraciones incrementales: %v", err)
	}

	return nil
}

// createMigrationsTable crea la tabla para rastrear migraciones aplicadas
func createMigrationsTable(db *sql.DB) error {
	...
	_, err := db.Exec(query)
	if err != nil {
		return fmt.Errorf("error al crear tabla schema_migrations: %v", err)
	}
	return nil
}

// runInitialMigration ejecuta la migración inicial (crear_tablas.sql)
func runInitialMigration(db *sql.DB) error {
	// Verificar si ya se ejecutó
	var exists bool
	err := db.QueryRow("SELECT EXISTS(SELECT 1 FROM schema_migrations WHERE version = 'inicial')").Scan(&exists)
	if err != nil {
		return fmt.Errorf("error al verificar migración inicial: %v", err)
	}

	if exists {
		log.Println("ℹ️ Migración inicial ya aplicada, saltando...")
		return nil
	}

	// Buscar archivo crear_tablas.sql
	migrationPaths := []string{
		"./migrations/crear_tablas.sql",
		"/app/migrations/crear_tablas. sql",
	}

	var migrationFile []byte
	var foundPath string

	for _, path := range migrationPaths {
		if _, err := os.Stat(path); err == nil {
			migrationFile, err = os.ReadFile(path)
			if err != nil {
	...
	if foundPath == "" {
		return fmt.Errorf("no se encontró archivo crear_tablas.sql en ninguna ubicación")
	}

	log.Printf("📋 Ejecutando migración inicial desde:  %s", foundPath)

	// Ejecutar SQL
	_, err = db.Exec(string(migrationFile))
	if err != nil {
		return fmt.Errorf("error al ejecutar migración inicial: %v", err)
	}

	// Registrar migración
	_, err = db.Exec("INSERT INTO schema_migrations (version) VALUES ('inicial')")
	if err != nil {
		return fmt.Errorf("error al registrar migración inicial: %v", err)
	}

	log.Println("✅ Migración inicial completada")
	return nil
}

// runIncrementalMigrations ejecuta migraciones incrementales automáticamente
func runIncrementalMigrations(db *sql.DB) error {
	migrations := []struct {
		...
		{
			version: "ampliar_columnas_cifrado",
			sql: `
				-- Ampliar columnas para soportar datos cifrados
				ALTER TABLE cliente ALTER COLUMN correo TYPE VARCHAR(255);
				ALTER TABLE cliente ALTER COLUMN numero_documento TYPE VARCHAR(100);
				ALTER TABLE cliente ALTER COLUMN numero_celular TYPE VARCHAR(150);
				ALTER TABLE cliente ALTER COLUMN contrasena TYPE VARCHAR(500);
			`,
		},
		// ✅ Agregar más migraciones aquí en el futuro
		// {
		// 	version:  "agregar_columna_xxx",
		// 	sql:  `ALTER TABLE xxx ADD COLUMN yyy VARCHAR(50);`,
		// },
	}
	...
	for _, migration := range migrations {
		...
		if err != nil {
			return fmt.Errorf("error al verificar migración %s: %v", migration.version, err)
		}

		if exists {
			log.Printf("ℹ️ Migración '%s' ya aplicada, saltando...", migration.version)
			continue
		}

		log.Printf("📋 Aplicando migración:  %s", migration.version)

		// Ejecutar SQL
		_, err = db.Exec(migration.sql)
		if err != nil {
			return fmt.Errorf("error al ejecutar migración %s: %v", migration.version, err)
		}

		// Registrar migración
		_, err = db.Exec("INSERT INTO schema_migrations (version) VALUES ($1)", migration.version)
		if err != nil {
			return fmt.Errorf("error al registrar migración %s:  %v", migration.version, err)
		}

		log.Printf("✅ Migración '%s' completada", migration.version)
	}
