...
func (s *ReservaService) ReservarConMercadoPago(
	...
	// Establecer fecha de expiración predeterminada (24 horas)
	fechaExpiracion := time.Now().Add(24 * time.Hour)
	err = s.reservaRepo.SetExpirationDate(idReserva, fechaExpiracion)
	if err != nil {
		fmt.Printf("Error al establecer fecha de expiración inicial para reserva %d: %v\n", idReserva, err)
...
func (s *ReservaService) ConfirmarPagoReserva(idReserva int, idTransaccion string, monto float64) error {
	...
	// Confirmar la reserva y registrar el pago en una operación
	err = s.reservaRepo.ConfirmarReservaConPago(idReserva, idTransaccion, monto)
	if err != nil {
		return fmt.Errorf("error al confirmar la reserva con pago: %v", err)
	}
...
// VerificarYConfirmarPago verifica el estado del pago con Mercado Pago y actualiza la reserva si es necesario
func (s *ReservaService) VerificarYConfirmarPago(idReserva int, status string, paymentID string, preferenceID string, mercadoPagoService *MercadoPagoService) (string, error) {
	// Verificar que la reserva existe
	reserva, err := s.GetByID(idReserva)
	if err != nil {
		return "", fmt.Errorf("error al obtener la reserva: %v", err)
	...
	if status != "" {
		...
		if estadoPago == "pending" || estadoPago == "in_process" {
			err = s.reservaRepo.UpdateExpirationByPaymentMethod(idReserva, paymentInfo.PaymentMethodId)
			if err != nil {
				fmt.Printf("Error al actualizar fecha de expiración por método de pago: %v\n", err)
				// No fallar la operación por esto
			}
		}
	} else if preferenceID != "" {
		// Si tenemos preference_id, verificar con Mercado Pago
		status, paymentMethodId, _, err := mercadoPagoService.VerificarEstadoPago(preferenceID, "")
		if err != nil {
			return "", fmt.Errorf("error al verificar estado del pago: %v", err)
		}
		estadoPago = status
		...
		// Actualizar fecha de expiración según el método de pago
		if estadoPago == "pending" || estadoPago == "in_process" {
			err = s.reservaRepo.UpdateExpirationByPaymentMethod(idReserva, paymentMethodId)
			if err != nil {
				fmt.Printf("Error al actualizar fecha de expiración por método de pago: %v\n", err)
				// No fallar la operación por esto
			}
...
func (s *ReservaService) SetExpirationDate(id int, fechaExpiracion time.Time) error {
	// Verificar que la reserva existe
	...
	// Solo establecer fecha de expiración para reservas en estado RESERVADO
	if reserva.Estado != "RESERVADO" {
		return errors.New("solo se puede establecer fecha de expiración para reservas en estado RESERVADO")
	}

	return s.reservaRepo.SetExpirationDate(id, fechaExpiracion)
}

// GetExpiredReservations obtiene todas las reservas expiradas
func (s *ReservaService) GetExpiredReservations() ([]*entidades.Reserva, error) {
	return s.reservaRepo.GetExpiredReservations()
}

// ProcessExpiredReservations cancela todas las reservas expiradas
func (s *ReservaService) ProcessExpiredReservations() (int, error) {
	return s.reservaRepo.CancelarReservasExpiradas()
}

// UpdateExpirationByPaymentMethod actualiza la fecha de expiración según el método de pago
func (s *ReservaService) UpdateExpirationByPaymentMethod(idReserva int, metodoPago string) error {
	// Verificar que la reserva existe
	reserva, err := s.reservaRepo.GetByID(idReserva)
	if err != nil {
		return errors.New("la reserva especificada no existe")
	...
	// Solo actualizar fecha de expiración para reservas en estado RESERVADO
	if reserva.Estado != "RESERVADO" {
		return errors.New("solo se puede actualizar fecha de expiración para reservas en estado RESERVADO")
	}

	return s.reservaRepo.UpdateExpirationByPaymentMethod(idReserva, metodoPago)
}

// VerifyReservationExpiration verifica si una reserva ha expirado
func (s *ReservaService) VerifyReservationExpiration(id int) (*entidades.ReservaExpiredResponse, error) {
	// Verificar que la reserva existe
	reserva, err := s.reservaRepo.GetByID(id)
	if err != nil {
	...
	// Preparar respuesta
	response := &entidades.ReservaExpiredResponse{
	...
	// Si no tiene fecha de expiración o no está en estado RESERVADO, no está expirada
	if reserva.FechaExpiracion == nil || reserva.Estado != "RESERVADO" {
		return response, nil
	}

	// Asignar fecha de expiración a la respuesta
	response.FechaExpiracion = *reserva.FechaExpiracion

	// Verificar si la fecha de expiración ha pasado
	if time.Now().After(*reserva.FechaExpiracion) {
		response.Expirada = true
		response.Mensaje = "Reserva expirada"

		// Cancelar automáticamente si ha expirado
		err = s.reservaRepo.UpdateEstado(id, "CANCELADA")
		if err != nil {
			return nil, fmt.Errorf("error al cancelar reserva expirada: %v", err)
		}
