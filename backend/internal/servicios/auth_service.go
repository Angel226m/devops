...
func (s *AuthService) Login(loginReq *entidades.LoginRequest, rememberMe bool) (*entidades.LoginResponse, error) {
	...
	if loginReq.Correo == "admin@sistema-tours.com" && loginReq.Contrasena == "admin123" {
		...
		// 🔥 USAR utils.GenerateJWT en lugar de métodos internos
		token, err := utils.GenerateJWT(usuario, s.config)
		if err != nil {
			return nil, err
		}

		// 🔥 USAR utils.GenerateRefreshToken en lugar de métodos internos
...
func (s *AuthService) RefreshToken(refreshToken string) (*entidades.LoginResponse, error) {
	...
	// 🔥 USAR utils.GenerateJWT - SIEMPRE usar el rol de la BD
	usuario.Rol = usuario.Rol // Asegurar que usamos el rol actual de la BD
...
// 🔥 MÉTODO SIMPLIFICADO: GenerateTokensForAdminWithSede
func (s *AuthService) GenerateTokensForAdminWithSede(userID int, sedeID int, rememberMe bool) (string, string, error) {
	// Obtener usuario para generar tokens con rol actualizado
	usuario, err := s.GetUserByID(userID)
	if err != nil {
		return "", "", err
	}
	...
	// Generar token con sede
	token, err := utils.GenerateJWT(usuario, s.config, sedeID)
	if err != nil {
		return "", "", err
	}

	// Generar refresh token con sede
...
// 🔥 MÉTODO SIMPLIFICADO: GenerateTokensWithoutDb
func (s *AuthService) GenerateTokensWithoutDb(userID int, userRole string, rememberMe bool) (string, string, error) {
	// 🔥 SIEMPRE obtener el usuario de la BD para garantizar consistencia
	usuario, err := s.GetUserByID(userID)
	if err != nil {
	...
	fmt.Printf("GenerateTokensWithoutDb: Usando rol de BD: %s (ignorando parámetro: %s)\n", usuario.Rol, userRole)

	// Generar tokens con el rol correcto
	token, err := utils.GenerateJWT(usuario, s.config)
	if err != nil {
		return "", "", err
	}
...
// 🔥 MÉTODO SIMPLIFICADO: GenerateTokensForUserWithSede
func (s *AuthService) GenerateTokensForUserWithSede(userID int, sedeID int, userRole string, rememberMe bool) (string, string, error) {
...
func (s *AuthService) GenerateTokensWithSede(userID int, sedeID int, rememberMe bool) (string, string, error) {
	return s.GenerateTokensForAdminWithSede(userID, sedeID, rememberMe)
}
