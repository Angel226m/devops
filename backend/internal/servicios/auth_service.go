...
func (s *AuthService) Login(loginReq *entidades.LoginRequest, rememberMe bool) (*entidades.LoginResponse, error) {
	...
	if loginReq.Correo == "admin@sistema-tours.com" && loginReq.Contrasena == "admin123" {
		...
		// 🔥 USAR utils.GenerateJWT en lugar de método interno
		token, err := utils.GenerateJWT(usuario, s.config)
		if err != nil {
			return nil, err
		}

		// 🔥 USAR utils.GenerateRefreshToken en lugar de método interno
		refreshToken, err := utils.GenerateRefreshToken(usuario, s.config, rememberMe)
		if err != nil {
			return nil, err
		}
		...
		fmt.Printf("🔥 Login exitoso para admin hardcodeado - Rol: %s\n", usuario.Rol)
		return loginResp, nil
	}
	...
	// 🔥 USAR utils.GenerateJWT
	token, err := utils.GenerateJWT(usuario, s.config)
	if err != nil {
		return nil, err
	}

	// 🔥 USAR utils.GenerateRefreshToken
	refreshToken, err := utils.GenerateRefreshToken(usuario, s.config, rememberMe)
	if err != nil {
		return nil, err
	}
	...
	fmt.Printf("🔥 Login exitoso para usuario - ID: %d, Rol: %s\n", usuario.ID, usuario.Rol)
	return loginResp, nil
}

// 🔥 ELIMINAR generateAccessToken y generateRefreshToken internos
// Ya no los necesitamos porque usamos utils.GenerateJWT y utils.GenerateRefreshToken
...
func (s *AuthService) RefreshToken(refreshToken string) (*entidades.LoginResponse, error) {
	...
	// Obtener usuario SIEMPRE de la BD para tener el rol actualizado
	usuario, err := s.usuarioRepo.GetByID(claims.UserID)
	if err != nil {
		return nil, err
	}
	...
	// 🔥 USAR utils.GenerateJWT - SIEMPRE con el rol actual de la BD
	...
	// 🔥 USAR utils.GenerateRefreshToken
	newRefreshToken, err := utils.GenerateRefreshToken(usuario, s.config, isRememberMe)
	if err != nil {
		return nil, err
	}
	...
	fmt.Printf("🔥 Refresh exitoso - Usuario ID: %d, Rol actual: %s\n", usuario.ID, usuario.Rol)
	return loginResp, nil
}
...
// 🔥 UNIFICAR GenerateTokensWithSede
func (s *AuthService) GenerateTokensWithSede(userID int, sedeID int, rememberMe bool) (string, string, error) {
	// Obtener usuario para verificar rol
	usuario, err := s.GetUserByID(userID)
	if err != nil {
	...
	// 🔥 USAR utils.GenerateJWT con sede
	token, err := utils.GenerateJWT(usuario, s.config, sedeID)
	if err != nil {
		return "", "", err
	}

	// 🔥 USAR utils.GenerateRefreshToken con sede
	refreshToken, err := utils.GenerateRefreshToken(usuario, s.config, rememberMe, sedeID)
	if err != nil {
		return "", "", err
	}

	fmt.Printf("🔥 Tokens con sede generados - Usuario: %d, Rol: %s, Sede: %d\n", userID, usuario.Rol, sedeID)
	return token, refreshToken, nil
}

// 🔥 SIMPLIFICAR GenerateTokensForAdminWithSede
func (s *AuthService) GenerateTokensForAdminWithSede(userID int, sedeID int, rememberMe bool) (string, string, error) {
	return s.GenerateTokensWithSede(userID, sedeID, rememberMe)
}

// 🔥 CORREGIR GenerateTokensWithoutDb para garantizar consistencia
func (s *AuthService) GenerateTokensWithoutDb(userID int, userRole string, rememberMe bool) (string, string, error) {
	// OBTENER SIEMPRE el usuario de la BD para verificar el rol actual
	usuario, err := s.GetUserByID(userID)
	if err != nil {
		return "", "", fmt.Errorf("no se pudo verificar el usuario %d: %w", userID, err)
	}

	// USAR SIEMPRE EL ROL DE LA BD
	if usuario.Rol != userRole {
		fmt.Printf("⚠️ Discrepancia de rol detectada - Parámetro: %s, BD: %s. Usando BD.\n", userRole, usuario.Rol)
	}

	// 🔥 USAR utils.GenerateJWT con el usuario completo de la BD
	token, err := utils.GenerateJWT(usuario, s.config)
	if err != nil {
		return "", "", err
	}
	...
	fmt.Printf("🔥 Tokens sin DB generados - Usuario: %d, Rol BD: %s\n", userID, usuario.Rol)
	return token, refreshToken, nil
}

// 🔥 CORREGIR GenerateTokensForUserWithSede
func (s *AuthService) GenerateTokensForUserWithSede(userID int, sedeID int, userRole string, rememberMe bool) (string, string, error) {
	...
	fmt.Printf("🔥 Tokens usuario con sede generados - Usuario: %d, Rol: %s, Sede: %d\n", userID, usuario.Rol, sedeID)
	return token, refreshToken, nil
}
