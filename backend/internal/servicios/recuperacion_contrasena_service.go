...
func (s *RecuperacionContrasenaService) SolicitarRecuperacion(solicitud *entidades.SolicitudRecuperacion) error {
	...
	// Buscar la entidad por correo según su tipo
	if solicitud.TipoEntidad == entidades.EntidadUsuario {
		// ✅ Para usuarios NO ciframos (asumiendo que no están cifrados)
		usuario, err := s.usuarioRepo.GetByEmail(solicitud.Correo)
		if err == nil {
			entidadID = usuario.ID
			nombreDestinatario = fmt.Sprintf("%s %s", usuario.Nombres, usuario.Apellidos)
		...
		// ✅ NUEVO:   Cifrar el correo antes de buscar
		log.Printf("Cifrando correo del cliente para búsqueda...")
		correoCifrado, err := utils.EncryptCorreo(solicitud.Correo)
		if err != nil {
			log.Printf("Error al cifrar correo para búsqueda: %v", err)
			// ⚠️ Por seguridad, no revelamos el error real
			return nil
		}

		log.Printf("Buscando cliente con correo cifrado...")
		cliente, err := s.clienteRepo.GetByCorreo(correoCifrado)
		if err == nil {
			entidadID = cliente.ID

			// ✅ Descifrar datos del cliente para usarlos
			if err := descifrarDatosCliente(cliente); err != nil {
				log.Printf("⚠️ Error al descifrar datos del cliente: %v", err)
				// Intentar usar los datos cifrados como fallback
			}
	...
	// Enviar correo con el token (usar el correo original sin cifrar)
	err = s.emailService.EnviarCorreoRecuperacionContrasena(
		solicitud.Correo, // ✅ Usar correo original (sin cifrar)
		token,
		nombreDestinatario,
		string(solicitud.TipoEntidad),
	)
...
// ═══════════════════════════════════════════════════════════════
// ✅ FUNCIÓN AUXILIAR PARA DESCIFRAR DATOS DEL CLIENTE
// ═══════════════════════════════════════════════════════════════

// descifrarDatosCliente descifra los datos sensibles de un cliente
func descifrarDatosCliente(cliente *entidades.Cliente) error {
	if cliente == nil {
