...
func (c *MercadoPagoController) ProcessCardPayment(ctx *gin.Context) {
	fmt.Printf("💳 ProcessCardPayment: MÉTODO LLAMADO CORRECTAMENTE\n")
	fmt.Printf("   - Method: %s\n", ctx.Request.Method)
	...
	// 🔧 LEER EL BODY ANTES DE HACER BIND
	bodyBytes, err := ctx.GetRawData()
	if err != nil {
		fmt.Printf("❌ Error al leer body: %v\n", err)
		ctx.JSON(http.StatusBadRequest, utils.ErrorResponse("Error al leer datos", err))
		return
	}

	fmt.Printf("📥 Body recibido: %s\n", string(bodyBytes))

	// 🔧 RESTAURAR EL BODY PARA EL BIND
	ctx.Request.Body = io.NopCloser(bytes.NewReader(bodyBytes))

	if err := ctx.ShouldBindJSON(&request); err != nil {
		fmt.Printf("❌ Error en ShouldBindJSON: %v\n", err)
	...
	fmt.Printf("✅ Datos recibidos correctamente:\n")
	fmt.Printf("   - Token: %s\n", request.Token)
	...
	// 🔧 VERIFICAR QUE LA RESERVA EXISTE
	fmt.Printf("🔍 Buscando reserva con ID: %d\n", request.ReservaID)
	reserva, err := c.reservaService.GetByID(request.ReservaID)
	if err != nil {
		fmt.Printf("❌ Reserva no encontrada - ID: %d, Error: %v\n", request.ReservaID, err)
		ctx.JSON(http.StatusNotFound, utils.ErrorResponse("Reserva no encontrada", err))
		return
	}

	fmt.Printf("✅ Reserva encontrada - ID: %d, Estado: %s, Total: %.2f, ClienteID: %d\n",
		reserva.ID, reserva.Estado, reserva.TotalPagar, reserva.IDCliente)

	// 🔧 VERIFICAR QUE EL CLIENTE EXISTE
	fmt.Printf("🔍 Buscando cliente con ID: %d\n", reserva.IDCliente)
	cliente, err := c.clienteService.GetByID(reserva.IDCliente)
	if err != nil {
		fmt.Printf("❌ Cliente no encontrado - ID: %d, Error: %v\n", reserva.IDCliente, err)
		ctx.JSON(http.StatusInternalServerError, utils.ErrorResponse("Error al obtener cliente", err))
		return
	}

	fmt.Printf("✅ Cliente encontrado - ID: %d, Email: %s, Nombre: %s %s\n",
		cliente.ID, cliente.Correo, cliente.Nombres, cliente.Apellidos)

	// 🔧 PROCESAR PAGO CON MANEJO DE ERRORES MEJORADO
	fmt.Printf("🚀 Iniciando procesamiento de pago con MercadoPago...\n")
	...
	fmt.Printf("📝 Registrando pago en el sistema...\n")
	nuevoPago := &entidades.NuevoPagoRequest{
	...
	if err != nil {
		...
		fmt.Printf("✅ Pago registrado con ID: %d\n", idPago)
		...
		if err != nil {
			...
			fmt.Printf("✅ Estado del pago actualizado a: %s\n", estadoPago)
		}
	}

	// 🔧 ACTUALIZAR ESTADO DE RESERVA
	if paymentResp.Status == "approved" {
		fmt.Printf("✅ Pago aprobado, actualizando reserva a CONFIRMADA...\n")
		err = c.reservaService.UpdateEstado(request.ReservaID, "CONFIRMADA")
		if err != nil {
			fmt.Printf("❌ Error al actualizar reserva: %v\n", err)
		} else {
			fmt.Printf("✅ Reserva actualizada a CONFIRMADA\n")
		}
	}
	...
	if idPago > 0 {
		responseData["pago_id"] = idPago
	}

	fmt.Printf("📤 Enviando respuesta exitosa al frontend\n")
	ctx.JSON(http.StatusOK, utils.SuccessResponse("Pago procesado exitosamente", responseData))
}
