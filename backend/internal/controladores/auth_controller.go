...
func (c *AuthController) RefreshToken(ctx *gin.Context) {
	...
	// CORREGIDO: Obtener siempre el usuario y su rol actual de la base de datos
	usuario, err := c.authService.GetUserByID(userID)
	if err != nil {
		fmt.Printf("RefreshToken: Error al obtener usuario: %v\n", err)
		ctx.JSON(http.StatusUnauthorized, utils.ErrorResponse("Usuario no encontrado", err))
		return
	}

	// CORREGIDO: Usar SIEMPRE el rol actual de la BD, ignorando el del token
	userRole := usuario.Rol
	fmt.Printf("RefreshToken: Usando rol de BD: %s (ignorando rol del token: %s)\n", userRole, claims.Role)
	...
	// Si tiene sede seleccionada y es ADMIN (verificando el rol de la BD)
	if sedeID > 0 && userRole == "ADMIN" {
		token, newRefreshToken, err = c.authService.GenerateTokensForAdminWithSede(userID, sedeID, isRememberMe)
	} else if userRole != "ADMIN" && usuario.IdSede != nil {
		// Usuario no admin con sede asignada en BD
		token, newRefreshToken, err = c.authService.GenerateTokensForUserWithSede(userID, *usuario.IdSede, userRole, isRememberMe)
	} else {
		// Sin sede seleccionada - USAR EL ROL DE LA BD
	...
	// Crear objeto de usuario para la respuesta usando los datos completos
...
func (c *AuthController) CheckStatus(ctx *gin.Context) {
	...
	// CORREGIDO: Verificar si hay discrepancia entre el rol del token y el de la BD
	if roleExists && usuario.Rol != userRole.(string) {
		...
		// Regenerar token con el rol correcto
		...
		if usuario.Rol == "ADMIN" && sedeID > 0 {
			token, newRefreshToken, err = c.authService.GenerateTokensForAdminWithSede(usuario.ID, sedeID, isRememberMe)
		} else if usuario.Rol != "ADMIN" && usuario.IdSede != nil {
			token, newRefreshToken, err = c.authService.GenerateTokensForUserWithSede(usuario.ID, *usuario.IdSede, usuario.Rol, isRememberMe)
		} else {
			token, newRefreshToken, err = c.authService.GenerateTokensWithoutDb(usuario.ID, usuario.Rol, isRememberMe)
		}
