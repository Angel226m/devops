...
func GenerateJWT(usuario *entidades.Usuario, config *config.Config, sedeID ...int) (string, error) {
	...
	claims := TokenClaims{
		UserID: usuario.ID,
		Email:  usuario.Correo, // 🔥 SIEMPRE incluir email
		Role:   usuario.Rol,    // 🔥 USAR EL ROL DEL USUARIO PASADO
		RegisteredClaims: jwt.RegisteredClaims{
			ExpiresAt: jwt.NewNumericDate(time.Now().Add(time.Minute * 15)), // 15 minutos
			IssuedAt:  jwt.NewNumericDate(time.Now()),
			NotBefore: jwt.NewNumericDate(time.Now()),
	...
	if len(sedeID) > 0 && sedeID[0] > 0 {
		claims.SedeID = sedeID[0]
		fmt.Printf("GenerateJWT: Token generado con sede %d para usuario %d (rol: %s)\n", sedeID[0], usuario.ID, usuario.Rol)
	} else {
		fmt.Printf("GenerateJWT: Token generado sin sede para usuario %d (rol: %s)\n", usuario.ID, usuario.Rol)
	}
...
func GenerateRefreshToken(usuario *entidades.Usuario, config *config.Config, rememberMe bool, sedeID ...int) (string, error) {
	...
	// Imprimir información sobre el token
	fmt.Printf("GenerateRefreshToken: Generando para usuario ID: %d, Rol: %s\n", usuario.ID, usuario.Rol)

	// Crear token con los claims
	token := jwt.NewWithClaims(jwt.SigningMethodHS256, claims)

	// Firmar token con la llave secreta para refresh tokens
	tokenString, err := token.SignedString([]byte(config.JWTRefreshSecret))
	if err != nil {
		return "", err
	}

	fmt.Printf("GenerateRefreshToken: Token generado exitosamente para rol %s\n", usuario.Rol)
	return tokenString, nil
}
