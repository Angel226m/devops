 /*
const en = {
  menu: {
    inicio: 'Home',
    tours: 'Tours',
    sedes: 'Locations',
    sobreNosotros: 'About Us',
    contacto: 'Contact',
    preguntasFrecuentes: 'FAQ'
  },
  autenticacion: {
    ingresar: 'Sign In',
    registrarse: 'Sign Up',
    cerrarSesion: 'Sign Out'
  },
  idioma: {
    cambiar: 'Change language'
  },
  hero: {
    slide1: {
      titulo: 'Discover the Marine Beauty of Pisco',
      subtitulo: 'Unique experiences on the coast of Peru'
    },
    slide2: {
      titulo: 'Live Unforgettable Adventures',
      subtitulo: 'Exclusive tours for the whole family'
    },
    slide3: {
      titulo: 'Nature and Biodiversity',
      subtitulo: 'Explore Peru\'s most impressive natural reserve'
    },
    botonExplorar: 'Explore Tours',
    botonContacto: 'Contact Us'
  },
  inicio: {
    toursDestacados: 'Featured Tours',
    toursDestacadosDescripcion: 'Discover our most popular tours and enjoy unforgettable experiences.',
    verTodosTours: 'View all tours',
    porQueElegirnos: 'Why Choose Us?',
    porQueElegirnosDescripcion: 'We offer unique experiences with the best service and safety.',
    seguridad: 'Guaranteed Safety',
    seguridadDescripcion: 'Your safety is our priority. Our boats meet all safety standards.',
    calidad: 'Superior Quality',
    calidadDescripcion: 'First-class service with expert guides and modern, comfortable boats.',
    experiencia: 'Unique Experiences',
    experienciaDescripcion: 'Unforgettable experiences designed to exceed your expectations.',
    testimonios: 'What Our Customers Say',
    testimoniosDescripcion: 'Hear about the experiences of those who have already enjoyed our tours.',
    socios: 'Our Partners',
    sociosDescripcion: 'We work with the best companies to provide you with exceptional service.'
  },
  footer: {
    descripcion: 'We are leaders in maritime tours in Pisco, Peru, offering unique experiences to explore the natural beauty of our coasts.',
    enlacesRapidos: 'Quick Links',
    contacto: 'Contact',
    horario: 'Hours',
    boletin: 'Newsletter',
    boletinDescripcion: 'Subscribe to our newsletter to receive offers and news.',
    correoPlaceholder: 'Your email address',
    suscribirse: 'Subscribe',
    derechosReservados: 'All rights reserved.',
    terminos: 'Terms and Conditions',
    privacidad: 'Privacy Policy',
    cookies: 'Cookie Policy'
  },
  boletin: {
    titulo: 'Stay Informed',
    descripcion: 'Subscribe to our newsletter to receive exclusive offers, travel tips, and latest news.',
    beneficio1Titulo: 'Exclusive Offers',
    beneficio1Descripcion: 'Access special promotions and discounts for subscribers only.',
    beneficio2Titulo: 'Instant Updates',
    beneficio2Descripcion: 'Be the first to know about new tours and activities.',
    formTitulo: 'Join our community',
    correo: 'Email Address',
    correoPlaceholder: 'example@email.com',
    nombre: 'Name',
    nombrePlaceholder: 'Your name',
    recibirOfertas: 'I want to receive special offers and news',
    suscribirse: 'Subscribe Now',
    privacidad: 'By subscribing, you accept our privacy policy and terms of use.',
    errorVacio: 'Please enter your email address.',
    errorInvalido: 'Please enter a valid email address.',
    errorServidor: 'There was an error processing your request. Please try again.',
    exitoTitulo: 'Thank you for subscribing!',
    exitoDescripcion: 'You will soon receive our news and exclusive offers in your email.',
    suscribirOtro: 'Subscribe another email'
  },
  404: {
    titulo: 'Page Not Found!',
    descripcion: 'Sorry, the page you are looking for does not exist or has been moved.',
    volverInicio: 'Back to home',
    contactarSoporte: 'Contact support',
    sugerencias: 'You might be interested in:'
  },
  validacion: {
    requerido: 'This field is required',
    correoInvalido: 'Please enter a valid email address',
    dniInvalido: 'ID must have 8 digits',
    contrasenaCorta: 'Password must be at least 6 characters long',
    contrasenaNoCoincide: 'Passwords do not match'
  },
  registro: {
    titulo: 'Create Your Account',
    descripcion: 'Join us and enjoy exclusive benefits.',
    nombres: 'First Name',
    apellidos: 'Last Name',
    tipoDocumento: 'Document Type',
    numeroDocumento: 'Document Number',
    correo: 'Email Address',
    telefono: 'Phone',
    contrasena: 'Password',
    confirmarContrasena: 'Confirm Password',
    aceptoTerminos: 'I accept the',
    terminos: 'terms and conditions',
    crearCuenta: 'Create Account',
    yaTienesCuenta: 'Already have an account?',
    iniciarSesion: 'Sign In',
    exitoso: 'Account created successfully!',
    error: 'There was an error creating your account. Please try again.',
    beneficio1Titulo: 'Faster bookings',
    beneficio1Descripcion: 'Save your data to book with just one click.',
    beneficio2Titulo: 'Tour history',
    beneficio2Descripcion: 'Access your tour and booking history.',
    beneficio3Titulo: 'Exclusive promotions',
    beneficio3Descripcion: 'Receive special offers only for registered users.'
  },
  recuperacion: {
    titulo: 'Recover Your Password',
    descripcion: 'Enter your email address and we will send you instructions to recover your password.',
    correo: 'Email Address',
    enviarInstrucciones: 'Send Instructions',
    recordaste: 'Remembered your password?',
    volverLogin: 'Back to sign in',
    correoRequerido: 'Please enter your email address.',
    correoInvalido: 'Please enter a valid email address.',
    errorEnvio: 'There was an error sending instructions. Please try again.',
    correoEnviado: 'Instructions Sent',
    instrucciones: 'We have sent instructions to recover your password to your email address. Please check your inbox.',
    volverInicio: 'Back to home',
    intentarOtroCorreo: 'Try with another email'
  },
  // Añade esta sección a tu objeto en
contacto: {
  titulo: 'Contact Us',
  subtitulo: 'We are here to answer any questions you may have',
  infoTitulo: 'Contact Information',
  infoDescripcion: 'Get in touch with us through any of these means. We will be happy to assist you.',
  telefono: 'Phone',
  email: 'Email',
  direccion: 'Address',
  horario: 'Business Hours',
  redesSociales: 'Follow us on social media',
  formTitulo: 'Send us a message',
  nombre: 'Full Name',
  asunto: 'Subject',
  mensaje: 'Message',
  sede: 'Location',
  sedePisco: 'Pisco',
  sedeParacas: 'Paracas',
  sedeSanAndres: 'San Andrés',
  enviar: 'Send Message',
  enviando: 'Sending...',
  mensajeExito: 'Message sent successfully! We will contact you soon.',
  faqTitulo: 'Frequently Asked Questions',
  faqDescripcion: 'Answers to the most common questions about our services.',
  faq1Pregunta: 'What are the tour hours?',
  faq1Respuesta: 'Our tours operate daily from 8:00 AM to 5:00 PM.',
  faq2Pregunta: 'How can I make a reservation?',
  faq2Respuesta: 'You can make a reservation directly on our website, by phone, or by visiting our offices.',
  faq3Pregunta: 'What if I want to cancel my reservation?',
  faq3Respuesta: 'Cancellations made more than 48 hours in advance receive a full refund.',
  faq4Pregunta: 'Do tours include transportation from the hotel?',
  faq4Respuesta: 'Yes, all our tours include pickup and return to hotels in the city center.',
  faq5Pregunta: 'What should I bring for maritime tours?',
  faq5Respuesta: 'We recommend bringing sunscreen, sunglasses, a cap or hat, and light clothing.',
  masPreguntas: 'Do you have more questions?',
  verMasPreguntas: 'View all frequently asked questions'
},
 conservacion: {
    titulo: 'Conservation and Social Responsibility',
    subtitulo: 'At Ocean Tours, we believe that responsible tourism should contribute to the preservation of marine ecosystems and the well-being of local communities.',
    
    proteccionTitulo: 'Protection of Marine Species',
    proteccionDescripcion: 'Our tours are designed to observe marine life without disturbing their natural habitat',
    
    iniciativasTitulo: 'Our Conservation Initiatives',
    iniciativa1Titulo: 'Beach Cleanup Program',
    iniciativa1Descripcion: 'We organize monthly beach cleanups with the participation of volunteers and our staff, collecting plastic waste and other debris.',
    iniciativa2Titulo: 'Environmental Education',
    iniciativa2Descripcion: 'During our tours, we educate visitors about the importance of marine conservation and how they can contribute to protecting these fragile ecosystems.',
    iniciativa3Titulo: 'Research Support',
    iniciativa3Descripcion: 'We collaborate with local institutions in monitoring studies of seabird populations and marine mammals such as sea lions and dolphins.',
    saberMas: 'Learn more about our initiatives',
    
    impactoTitulo: 'Social Impact',
    impactoSubtitulo: 'We work closely with local communities to ensure that tourism directly benefits the residents of Paracas.',
    
    empleoTitulo: 'Local Employment',
    empleoDescripcion: '90% of our staff are local residents. We provide ongoing training and professional development opportunities.',
    
    educacionTitulo: 'Community Education',
    educacionDescripcion: 'We offer free educational programs for local children on marine conservation and sustainable tourism.',
    
    artesanosTitulo: 'Support for Artisans',
    artesanosDescripcion: 'We promote the work of local artisans, selling their products in our stores and providing spaces to showcase their art.',
    
    especiesTitulo: 'Species We Protect',
    pingüinos: 'Humboldt Penguins',
    pingüinosDesc: 'Vulnerable species that inhabits the Paracas National Reserve',
    lobos: 'Sea Lions',
    lobosDesc: 'Important colonies in the Ballestas Islands that we constantly monitor',
    delfines: 'Dolphins and Whales',
    delfinesDesc: 'We respect strict observation guidelines to not disturb their natural behavior',
    
    uneteTitulo: 'Join Our Conservation Efforts',
    uneteDescripcion: 'By choosing Ocean Tours, you are directly contributing to our conservation initiatives. You can also actively participate in our volunteer programs.',
    contactar: 'Contact for Volunteering',
    explorarTours: 'Explore Sustainable Tours',
    
    certificacionesTitulo: 'Our Certifications and Partnerships'
  },

  reserva: {
    estados: {
      confirmed: 'Confirmed',
      reserved: 'Reserved',
      pending: 'Pending',
      cancelled: 'Cancelled',
      processed: 'Processed',
      voided: 'Voided'
    },
    gestionReservas: 'Manage your bookings and plan your next adventure',
    sinReservas: 'No bookings available',
    sinReservasEstado: 'No bookings in {estado} status',
    fechaNoDisponible: 'Date not available',
    fechaInvalida: 'Invalid date',
    hoy: 'Today',
    manana: 'Tomorrow',
    ayer: 'Yesterday',
    enDias: 'In {dias} days',
    haceDias: '{dias} days ago',
    tourNoEspecificado: 'Tour not specified',
    horarioNoEspecificado: 'Schedule not specified',
    noSesion: 'You are not logged in',
    iniciarSesionParaVerReservas: 'Log in to view your bookings',
    explorarTours: 'Explore our tours and book your next adventure',
    todos: 'All',
    verTours: 'View Tours',
    fecha: 'Date',
    pasajeros: 'Passengers',
    total: 'Total',
    estado: 'Status',
    acciones: 'Actions',
    verDetalles: 'View Details',
    filtrarPorEstado: 'Filter by status',
    reservadoEl: 'Booked on',
    estadisticasReservas: 'Booking Statistics'
  },
};

export default en;*/
const en = {
  menu: {
    inicio: 'Home',
    tours: 'Tours',
    sedes: 'Locations',
    sobreNosotros: 'About Us',
    contacto: 'Contact',
    preguntasFrecuentes: 'FAQ'
  },
  autenticacion: {
    ingresar: 'Sign In',
    registrarse: 'Sign Up',
    cerrarSesion: 'Sign Out'
  },
  idioma: {
    cambiar: 'Change language'
  },
  hero: {
    slide1: {
      titulo: 'Discover the Marine Beauty of Pisco',
      subtitulo: 'Unique experiences on the coast of Peru'
    },
    slide2: {
      titulo: 'Live Unforgettable Adventures',
      subtitulo: 'Exclusive tours for the whole family'
    },
    slide3: {
      titulo: 'Nature and Biodiversity',
      subtitulo: 'Explore Peru\'s most impressive natural reserve'
    },
    botonExplorar: 'Explore Tours',
    botonContacto: 'Contact Us'
  },
  inicio: {
    toursDestacados: 'Featured Tours',
    toursDestacadosDescripcion: 'Discover our most popular tours and enjoy unforgettable experiences.',
    verTodosTours: 'View all tours',
    porQueElegirnos: 'Why Choose Us?',
    porQueElegirnosDescripcion: 'We offer unique experiences with the best service and safety.',
    seguridad: 'Guaranteed Safety',
    seguridadDescripcion: 'Your safety is our priority. Our boats meet all safety standards.',
    calidad: 'Superior Quality',
    calidadDescripcion: 'First-class service with expert guides and modern, comfortable boats.',
    experiencia: 'Unique Experiences',
    experienciaDescripcion: 'Unforgettable experiences designed to exceed your expectations.',
    testimonios: 'What Our Customers Say',
    testimoniosDescripcion: 'Hear about the experiences of those who have already enjoyed our tours.',
    socios: 'Our Partners',
    sociosDescripcion: 'We work with the best companies to provide you with exceptional service.'
  },
  footer: {
    descripcion: 'We are leaders in maritime tours in Pisco, Peru, offering unique experiences to explore the natural beauty of our coasts.',
    enlacesRapidos: 'Quick Links',
    contacto: 'Contact',
    horario: 'Hours',
    boletin: 'Newsletter',
    boletinDescripcion: 'Subscribe to our newsletter to receive offers and news.',
    correoPlaceholder: 'Your email address',
    suscribirse: 'Subscribe',
    derechosReservados: 'All rights reserved.',
    terminos: 'Terms and Conditions',
    privacidad: 'Privacy Policy',
    cookies: 'Cookie Policy'
  },
  boletin: {
    titulo: 'Stay Informed',
    descripcion: 'Subscribe to our newsletter to receive exclusive offers, travel tips, and latest news.',
    beneficio1Titulo: 'Exclusive Offers',
    beneficio1Descripcion: 'Access special promotions and discounts for subscribers only.',
    beneficio2Titulo: 'Instant Updates',
    beneficio2Descripcion: 'Be the first to know about new tours and activities.',
    formTitulo: 'Join our community',
    correo: 'Email Address',
    correoPlaceholder: 'example@email.com',
    nombre: 'Name',
    nombrePlaceholder: 'Your name',
    recibirOfertas: 'I want to receive special offers and news',
    suscribirse: 'Subscribe Now',
    privacidad: 'By subscribing, you accept our privacy policy and terms of use.',
    errorVacio: 'Please enter your email address.',
    errorInvalido: 'Please enter a valid email address.',
    errorServidor: 'There was an error processing your request. Please try again.',
    exitoTitulo: 'Thank you for subscribing!',
    exitoDescripcion: 'You will soon receive our news and exclusive offers in your email.',
    suscribirOtro: 'Subscribe another email'
  },
  404: {
    titulo: 'Page Not Found!',
    descripcion: 'Sorry, the page you are looking for does not exist or has been moved.',
    volverInicio: 'Back to home',
    contactarSoporte: 'Contact support',
    sugerencias: 'You might be interested in:'
  },
  validacion: {
    requerido: 'This field is required',
    correoInvalido: 'Please enter a valid email address',
    dniInvalido: 'ID must have 8 digits',
    contrasenaCorta: 'Password must be at least 6 characters long',
    contrasenaNoCoincide: 'Passwords do not match'
  },
  registro: {
    titulo: 'Create Your Account',
    descripcion: 'Join us and enjoy exclusive benefits.',
    nombres: 'First Name',
    apellidos: 'Last Name',
    tipoDocumento: 'Document Type',
    numeroDocumento: 'Document Number',
    correo: 'Email Address',
    telefono: 'Phone',
    contrasena: 'Password',
    confirmarContrasena: 'Confirm Password',
    aceptoTerminos: 'I accept the',
    terminos: 'terms and conditions',
    crearCuenta: 'Create Account',
    yaTienesCuenta: 'Already have an account?',
    iniciarSesion: 'Sign In',
    exitoso: 'Account created successfully!',
    error: 'There was an error creating your account. Please try again.',
    beneficio1Titulo: 'Faster bookings',
    beneficio1Descripcion: 'Save your data to book with just one click.',
    beneficio2Titulo: 'Tour history',
    beneficio2Descripcion: 'Access your tour and booking history.',
    beneficio3Titulo: 'Exclusive promotions',
    beneficio3Descripcion: 'Receive special offers only for registered users.'
  },
  recuperacion: {
    titulo: 'Recover Your Password',
    descripcion: 'Enter your email address and we will send you instructions to recover your password.',
    correo: 'Email Address',
    enviarInstrucciones: 'Send Instructions',
    recordaste: 'Remembered your password?',
    volverLogin: 'Back to sign in',
    correoRequerido: 'Please enter your email address.',
    correoInvalido: 'Please enter a valid email address.',
    errorEnvio: 'There was an error sending instructions. Please try again.',
    correoEnviado: 'Instructions Sent',
    instrucciones: 'We have sent instructions to recover your password to your email address. Please check your inbox.',
    volverInicio: 'Back to home',
    intentarOtroCorreo: 'Try with another email'
  },
  contacto: {
    titulo: 'Contact Us',
    subtitulo: 'We are here to answer any questions you may have',
    infoTitulo: 'Contact Information',
    infoDescripcion: 'Get in touch with us through any of these means. We will be happy to assist you.',
    telefono: 'Phone',
    email: 'Email',
    direccion: 'Address',
    horario: 'Business Hours',
    redesSociales: 'Follow us on social media',
    formTitulo: 'Send us a message',
    nombre: 'Full Name',
    asunto: 'Subject',
    mensaje: 'Message',
    sede: 'Location',
    sedePisco: 'Pisco',
    sedeParacas: 'Paracas',
    sedeSanAndres: 'San Andrés',
    enviar: 'Send Message',
    enviando: 'Sending...',
    mensajeExito: 'Message sent successfully! We will contact you soon.',
    faqTitulo: 'Frequently Asked Questions',
    faqDescripcion: 'Answers to the most common questions about our services.',
    faq1Pregunta: 'What are the tour hours?',
    faq1Respuesta: 'Our tours operate daily from 8:00 AM to 5:00 PM.',
    faq2Pregunta: 'How can I make a reservation?',
    faq2Respuesta: 'You can make a reservation directly on our website, by phone, or by visiting our offices.',
    faq3Pregunta: 'What if I want to cancel my reservation?',
    faq3Respuesta: 'Cancellations made more than 48 hours in advance receive a full refund.',
    faq4Pregunta: 'Do tours include transportation from the hotel?',
    faq4Respuesta: 'Yes, all our tours include pickup and return to hotels in the city center.',
    faq5Pregunta: 'What should I bring for maritime tours?',
    faq5Respuesta: 'We recommend bringing sunscreen, sunglasses, a cap or hat, and light clothing.',
    masPreguntas: 'Do you have more questions?',
    verMasPreguntas: 'View all frequently asked questions'
  },
  conservacion: {
    titulo: 'Conservation and Social Responsibility',
    subtitulo: 'At Ocean Tours, we believe that responsible tourism should contribute to the preservation of marine ecosystems and the well-being of local communities.',
    proteccionTitulo: 'Protection of Marine Species',
    proteccionDescripcion: 'Our tours are designed to observe marine life without disturbing their natural habitat',
    iniciativasTitulo: 'Our Conservation Initiatives',
    iniciativa1Titulo: 'Beach Cleanup Program',
    iniciativa1Descripcion: 'We organize monthly beach cleanups with the participation of volunteers and our staff, collecting plastic waste and other debris.',
    iniciativa2Titulo: 'Environmental Education',
    iniciativa2Descripcion: 'During our tours, we educate visitors about the importance of marine conservation and how they can contribute to protecting these fragile ecosystems.',
    iniciativa3Titulo: 'Research Support',
    iniciativa3Descripcion: 'We collaborate with local institutions in monitoring studies of seabird populations and marine mammals such as sea lions and dolphins.',
    saberMas: 'Learn more about our initiatives',
    impactoTitulo: 'Social Impact',
    impactoSubtitulo: 'We work closely with local communities to ensure that tourism directly benefits the residents of Paracas.',
    empleoTitulo: 'Local Employment',
    empleoDescripcion: '90% of our staff are local residents. We provide ongoing training and professional development opportunities.',
    educacionTitulo: 'Community Education',
    educacionDescripcion: 'We offer free educational programs for local children on marine conservation and sustainable tourism.',
    artesanosTitulo: 'Support for Artisans',
    artesanosDescripcion: 'We promote the work of local artisans, selling their products in our stores and providing spaces to showcase their art.',
    especiesTitulo: 'Species We Protect',
    pingüinos: 'Humboldt Penguins',
    pingüinosDesc: 'Vulnerable species that inhabits the Paracas National Reserve',
    lobos: 'Sea Lions',
    lobosDesc: 'Important colonies in the Ballestas Islands that we constantly monitor',
    delfines: 'Dolphins and Whales',
    delfinesDesc: 'We respect strict observation guidelines to not disturb their natural behavior',
    uneteTitulo: 'Join Our Conservation Efforts',
    uneteDescripcion: 'By choosing Ocean Tours, you are directly contributing to our conservation initiatives. You can also actively participate in our volunteer programs.',
    contactar: 'Contact for Volunteering',
    explorarTours: 'Explore Sustainable Tours',
    certificacionesTitulo: 'Our Certifications and Partnerships'
  },
  reserva: {
    estados: {
      confirmed: 'Confirmed',
      reserved: 'Reserved',
      pending: 'Pending',
      cancelled: 'Cancelled',
      processed: 'Processed',
      voided: 'Voided'
    },
    gestionReservas: 'Manage your bookings and plan your next adventure',
    sinReservas: 'No bookings available',
    sinReservasEstado: 'No bookings in {estado} status',
    fechaNoDisponible: 'Date not available',
    fechaInvalida: 'Invalid date',
    hoy: 'Today',
    manana: 'Tomorrow',
    ayer: 'Yesterday',
    enDias: 'In {dias} days',
    haceDias: '{dias} days ago',
    tourNoEspecificado: 'Tour not specified',
    horarioNoEspecificado: 'Schedule not specified',
    noSesion: 'You are not logged in',
    iniciarSesionParaVerReservas: 'Log in to view your bookings',
    explorarTours: 'Explore our tours and book your next adventure',
    todos: 'All',
    verTours: 'View Tours',
    fecha: 'Date',
    pasajeros: 'Passengers',
    total: 'Total',
    estado: 'Status',
    acciones: 'Actions',
    verDetalles: 'View Details',
    filtrarPorEstado: 'Filter by status',
    reservadoEl: 'Booked on',
    estadisticasReservas: 'Booking Statistics'
  },
  // 🔧 NEW KEYS FOR RESERVATION FORM
  general: {
    cargando: 'Loading...',
    importante: 'Important',
    email: 'Email',
    horario: 'Schedule',
    atencion: 'Customer Service',
    horarioDetalles: 'Monday to Sunday 8am-8pm',
    continuar: 'Continue',
    volver: 'Go Back',
    procesando: 'Processing...',
    cerrar: 'Close'
  },
  tour: {
    reservarAhora: 'Book Now',
    porPersona: 'per person',
    fechaYHora: 'Date & Time',
    pasajeros: 'Passengers',
    seleccionaFechaHorario: 'Select date and time',
    fecha: 'Date',
    seleccionarFecha: 'Select date',
    horario: 'Schedule',
    seleccionarHorario: 'Select schedule',
    seleccionePrimeroFecha: 'Please select a date first',
    cargandoFechas: 'Loading available dates...',
    hayFechasDisponibles: 'Available dates',
    yMas: 'and {count} more',
    sinFechasDisponibles: 'No dates available',
    sinHorariosDisponiblesParaFecha: 'No schedules available for this date',
    sinCupo: 'No availability',
    disponible: 'Available',
    seleccionActual: 'Current selection',
    seleccionaPasajes: 'Select passengers',
    paquetesPromocion: 'Promotional Packages',
    pasajesIndividuales: 'Individual Tickets',
    noHayTiposPasajeDisponibles: 'No passenger types available',
    combinar: 'You can also combine',
    cantidad: 'Quantity',
    reducirCantidad: 'Reduce quantity',
    aumentarCantidad: 'Increase quantity',
    subtotal: 'Subtotal',
    totalPasajeros: 'Total passengers',
    totalPagar: 'Total to pay',
    continuarPago: 'Continue to payment',
    pagoSeguro: 'Secure Payment',
    metodosPago: 'Accepted payment methods',
    pagoCifrado: 'Encrypted and secure transactions',
    seleccionePasajeOPaquete: 'Select at least one ticket or package',
    noCupoDisponible: 'Not enough availability. Available',
    solicitado: 'requested',
    reserva24Horas: 'Reservations must be made at least 24 hours in advance.',
    pagoCompleto: 'Full payment is required at the time of booking.',
    notaPagoCompleto: 'Payment must be made in full at the time of confirming the reservation.',
    nota24Horas: 'Reservations must be made with a minimum of 24 hours in advance.',
    notaPrecios: 'Displayed prices include all applicable taxes.',
    notaConfirmacion: 'You will receive an email confirmation once payment is completed.',
    cargandoReserva: 'Loading booking system'
  },
  // 🔧 NEW KEYS FOR PAYMENT PROCESS
  pago: {
    titulo: 'Payment Process',
    subtitulo: 'Complete your booking by making the payment'
  }
};

export default en;