...
// 🔧 INTERFACES ACTUALIZADAS BASADAS EN LAS ENTIDADES PROPORCIONADAS
export interface CantidadPasaje {
  id_tipo_pasaje: number;
  cantidad: number;
  precio_unitario: number;
  subtotal?: number; // Calculable: cantidad * precio_unitario
  nombre_tipo?: string; // Opcional, si se resuelve desde backend o lookup
}
...
export interface ReservaDetallada {
  ...
  cliente?: {
    ...
    tipo_documento?: string; // Añadido para consistencia
    numero_documento?: string;
  };
  instancia?: {
    id_instancia: number;
  ...
  // Información adicional del tour (si se resuelve en backend)
  nombre_tour?: string;
  descripcion_tour?: string;
  hora_inicio_tour?: string;
  hora_fin_tour?: string;
...
const PaginaDetalleReserva = () => {
  ...
  // Cargar detalle de la reserva con logs de depuración
  useEffect(() => {
    if (id && autenticado) {
      console.log('🚀 Solicitando detalle de reserva con ID:', id);
      dispatch(obtenerDetalleReservaPorId(parseInt(id))).then((result) => {
        console.log('📥 Datos recibidos del backend:', result.payload);
        if (result.payload) {
          const reserva = result.payload as ReservaDetallada;
          console.log('👤 Pasajes individuales:', reserva.cantidad_pasajes);
          console.log('📦 Paquetes:', reserva.paquetes);
          console.log('🎯 Total pagar:', reserva.total_pagar);
          console.log('👨‍💼 Cliente:', reserva.cliente);
          console.log('🏢 Sede:', reserva.sede);
          console.log('🗺️ Instancia:', reserva.instancia);
        } else {
          console.error('❌ No se recibieron datos válidos');
        }
      });
    } else if (!autenticado) {
      console.warn('⚠️ No autenticado, redirigiendo a /ingresar');
      navigate('/ingresar');
    }
    
    // Limpiar al desmontar
    return () => {
      console.log('🧹 Limpiando detalle de reserva');
      dispatch(limpiarReservaDetalle());
    };
  }, [id, dispatch, autenticado, navigate]);

  // Función para formatear fechas - CORREGIDA Y MEJORADA
  const formatearFecha = (fechaStr?: string, formato: 'completo' | 'corto' | 'relativo' = 'completo'): string => {
    if (!fechaStr) return t('reservas.fechaNoDisponible');
  ...
  const formatearHora = (horaStr?: string): string => {
    if (!horaStr) return t('reservas.horaNoDisponible');
    try {
      if (horaStr.includes(':')) {
        const partes = horaStr.split(':');
        return `${partes[0]}:${partes[1]}`;
  ...
  // Obtener clase de color según el estado - MEJORADA CON TAILWIND CONSISTENTE
  const getEstadoClase = (estado: EstadoReserva): string => {
    const clases = {
      'CONFIRMADA': 'bg-emerald-100 text-emerald-800 border border-emerald-200',
      'CANCELADA': 'bg-rose-100 text-rose-800 border border-rose-200',
  ...
  // 🔧 CALCULAR TOTAL DE PASAJEROS - CORREGIDO Y CON LOGS
  const calcularTotalPasajeros = (): number => {
    if (!reserva) return 0;
    ...
    if (reserva.cantidad_pasajes) {
      totalIndividuales = reserva.cantidad_pasajes.reduce((sum, p) => {
        console.log(`👤 Individual: ID Tipo ${p.id_tipo_pasaje} = ${p.cantidad}`);
        return sum + p.cantidad;
      }, 0);
    }
    ...
    if (reserva.paquetes) {
      reserva.paquetes.forEach((paquete, index) => {
        console.log(`📦 Paquete ${index + 1} (ID ${paquete.id_paquete}, ${paquete.cantidad} unidades):`);
        if (paquete.desglose_pasajes) {
          const pasajerosPorPaquete = paquete.desglose_pasajes.reduce((sum, desglose) => {
            console.log(`  - ${desglose.tipo}: ${desglose.cantidad} por paquete`);
            return sum + desglose.cantidad;
          ...
          console.warn(`⚠️ Paquete ${paquete.id_paquete} sin desglose_pasajes`);
        }
      });
    }
  ...
  // 🔧 OBTENER DESGLOSE DETALLADO - CORREGIDO Y CON LOGS
  const getDesglosePasajeros = () => {
    if (!reserva) return {};
    ...
    if (reserva.cantidad_pasajes) {
      reserva.cantidad_pasajes.forEach(p => {
        const tipo = p.nombre_tipo || `Tipo ID ${p.id_tipo_pasaje}`; // Usar nombre si disponible, sino ID
        if (!desglose[tipo]) {
          desglose[tipo] = { individuales: 0, paquetes: 0, total: 0 };
        }
        desglose[tipo].individuales += p.cantidad;
    ...
    if (reserva.paquetes) {
      reserva.paquetes.forEach((paquete, paqIndex) => {
        console.log(`📦 Procesando paquete ${paqIndex + 1} (ID ${paquete.id_paquete}, ${paquete.cantidad} unidades):`);
        if (paquete.desglose_pasajes) {
          paquete.desglose_pasajes.forEach(desglosePaq => {
            const tipo = desglosePaq.tipo;
            const cantidadTotal = desglosePaq.cantidad * paquete.cantidad;
  ...
  // Estados de carga y error - MEJORADOS PARA UI
  if (!autenticado) {
    return (
      <div className="flex items-center justify-center min-h-[60vh] bg-gradient-to-br from-blue-50 via-sky-50 to-cyan-50">
        <div className="text-center bg-white p-8 rounded-2xl shadow-xl max-w-md border border-blue-100">
          <h2 className="text-2xl font-bold text-gray-900 mb-3">{t('reservas.necesitasIniciarSesion')}</h2>
          <p className="text-gray-600 mb-6">{t('reservas.iniciaSesionParaVerDetalles')}</p>
          <Link 
            to="/ingresar" 
            className="inline-flex items-center px-6 py-3 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-colors font-medium"
          >
            {t('reservas.iniciarSesion')}
          </Link>
        </div>
      </div>
    );
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-sky-50 to-cyan-50 py-8">
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto">
            <div className="bg-white rounded-2xl shadow-xl p-8">
              <Alerta mensaje={error || t('reservas.noSePudoCargar')} tipo="error" />
              <div className="text-center mt-6">
                <Link 
                  to="/mis-reservas" 
                  className="inline-flex items-center px-6 py-3 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-colors font-medium"
                >
                  {t('reservas.volverAMisReservas')}
                </Link>
              </div>
            </div>
          </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-sky-50 to-cyan-50 py-8">
      <div className="container mx-auto px-4">
        <div className="max-w-6xl mx-auto">
          ...
          {/* Header con navegación - MEJORADO CON RESPONSIVE */}
          <motion.div
            className="bg-white rounded-2xl shadow-xl p-6 mb-8 border border-blue-100"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            {/* Información principal del tour - MEJORADA CON GRID RESPONSIVE */}
            <motion.div
              className="lg:col-span-2 space-y-8"
              initial={{ opacity: 0, x: -20 }}
              animate={{ opacity: 1, x: 0 }}
            ...
            >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-4">
                        ...
                        <div className="flex items-center">
                          ...
                          <div>
                            <div className="text-sm text-gray-500 font-medium">{t('reservas.fechaDelTour')}</div>
                            <div className="text-lg font-semibold text-gray-900">
                              {formatearFecha(reserva.fecha_tour || reserva.instancia?.fecha_especifica, 'completo')}
                            </div>
                            <div className="text-sm text-blue-600">
              ...
              {/* 🎯 DESGLOSE DETALLADO DE PASAJEROS - MEJORADO CON TABLAS Y RESPONSIVE */}
              <div className="bg-white rounded-2xl shadow-xl p-8 border border-blue-100 overflow-x-auto">
                ...
                {/* 📊 RESUMEN POR TIPO DE PASAJERO - USANDO TABLA PARA MEJOR VISUALIZACIÓN */}
                {Object.keys(desglosePasajeros).length > 0 && (
                  <div className="mb-8">
                    <h4 className="text-lg font-semibold text-gray-900 mb-4 flex items-center">
                      <span className="w-6 h-6 rounded bg-blue-100 text-blue-600 text-sm flex items-center justify-center mr-2 font-bold">📊</span>
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                      <thead className="bg-blue-50">
                        <tr>
                          <th className="px-6 py-3 text-left text-xs font-medium text-blue-600 uppercase tracking-wider">{t('reservas.tipo')}</th>
                          <th className="px-6 py-3 text-left text-xs font-medium text-blue-600 uppercase tracking-wider">{t('reservas.individuales')}</th>
                          <th className="px-6 py-3 text-left text-xs font-medium text-blue-600 uppercase tracking-wider">{t('reservas.enPaquetes')}</th>
                          <th className="px-6 py-3 text-left text-xs font-medium text-blue-600 uppercase tracking-wider">{t('reservas.total')}</th>
                        </tr>
                      </thead>
                      <tbody className="bg-white divide-y divide-gray-200">
                        {Object.entries(desglosePasajeros).map(([tipo, datos]) => (
                          <tr key={tipo}>
                            <td className="px-6 py-4 whitespace-nowrap font-medium text-gray-900">{tipo}</td>
                            <td className="px-6 py-4 whitespace-nowrap text-blue-600">{datos.individuales}</td>
                            <td className="px-6 py-4 whitespace-nowrap text-emerald-600">{datos.paquetes}</td>
                            <td className="px-6 py-4 whitespace-nowrap font-bold text-purple-600">{datos.total}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                ...
                {/* 👤 PASAJEROS INDIVIDUALES - USANDO TABLA */}
                {reserva.cantidad_pasajes && reserva.cantidad_pasajes.length > 0 && (
                  <div className="mb-8">
                    <h4 className="text-lg font-semibold text-gray-900 mb-4 flex items-center">
                      <span className="w-6 h-6 rounded bg-blue-100 text-blue-600 text-sm flex items-center justify-center mr-2 font-bold">👤</span>
                      {t('reservas.pasajesIndividuales')} ({reserva.cantidad_pasajes.length} {t('reservas.tipos')})
                    </h4>
                    <table className="min-w-full divide-y divide-gray-200">
                      <thead className="bg-blue-50">
                        <tr>
                          <th className="px-6 py-3 text-left text-xs font-medium text-blue-600 uppercase tracking-wider">{t('reservas.tipo')}</th>
                          <th className="px-6 py-3 text-left text-xs font-medium text-blue-600 uppercase tracking-wider">{t('reservas.cantidad')}</th>
                          <th className="px-6 py-3 text-left text-xs font-medium text-blue-600 uppercase tracking-wider">{t('reservas.precioUnitario')}</th>
                          <th className="px-6 py-3 text-left text-xs font-medium text-blue-600 uppercase tracking-wider">{t('reservas.subtotal')}</th>
                        </tr>
                      </thead>
                      <tbody className="bg-white divide-y divide-gray-200">
                        {reserva.cantidad_pasajes.map((pasaje, index) => {
                          const subtotal = pasaje.subtotal || (pasaje.cantidad * pasaje.precio_unitario);
                          return (
                            <tr key={index}>
                              <td className="px-6 py-4 whitespace-nowrap font-medium text-gray-900">
                                {pasaje.nombre_tipo || `Tipo ID ${pasaje.id_tipo_pasaje}`}
                              </td>
                              <td className="px-6 py-4 whitespace-nowrap">{pasaje.cantidad}</td>
                              <td className="px-6 py-4 whitespace-nowrap">S/ {pasaje.precio_unitario.toFixed(2)}</td>
                              <td className="px-6 py-4 whitespace-nowrap font-bold text-blue-600">S/ {subtotal.toFixed(2)}</td>
                            </tr>
                          );
                        })}
                      </tbody>
                ...
                {/* 📦 PAQUETES - USANDO TABLA Y DESGLOSE INTERNO */}
                {reserva.paquetes && reserva.paquetes.length > 0 && (
                  <div>
                    <h4 className="text-lg font-semibold text-gray-900 mb-4 flex items-center">
                      <span className="w-6 h-6 rounded bg-emerald-100 text-emerald-600 text-sm flex items-center justify-center mr-2 font-bold">📦</span>
                      {t('reservas.paquetesAdquiridos')} ({reserva.paquetes.length} {t('reservas.paquetes')})
                    </h4>
                    <div className="space-y-6">
                      {reserva.paquetes.map((paquete, index) => {
                        const subtotal = paquete.subtotal || (paquete.cantidad * paquete.precio_unitario);
                        const pasajerosPorPaqueteUnitario = paquete.desglose_pasajes ? 
                          paquete.desglose_pasajes.reduce((sum, d) => sum + d.cantidad, 0) : 0;
                        const totalPasajerosTipoPaquete = pasajerosPorPaqueteUnitario * paquete.cantidad;
                        ...
                        return (
                          <div key={index} className="bg-emerald-50 rounded-xl p-6 border border-emerald-100">
                            <div className="flex flex-col md:flex-row justify-between items-start mb-4 gap-4">
                              <div className="flex items-center flex-1">
                                ...
                                <div>
                                  ...
                                  <div className="text-sm text-gray-600">
                                    <strong>{paquete.cantidad}</strong> {t('reservas.paquetes')} × <strong>{pasajerosPorPaqueteUnitario}</strong> {t('reservas.personasPorPaquete')} = <strong className="text-emerald-700">{totalPasajerosTipoPaquete} {t('reservas.personas')}</strong>
                                  </div>
                                </div>
                              </div>
                              <div className="text-right">
                            ...
                            {/* 🔍 DESGLOSE DEL PAQUETE - USANDO TABLA INTERNA */}
                            {paquete.desglose_pasajes && paquete.desglose_pasajes.length > 0 && (
                              <div className="mt-4 pt-4 border-t border-emerald-200">
                                <div className="text-sm font-medium text-emerald-700 mb-3 flex items-center">
                                  <svg className="h-4 w-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <table className="min-w-full divide-y divide-emerald-200">
                                  <thead className="bg-emerald-100">
                                    <tr>
                                      <th className="px-4 py-2 text-left text-xs font-medium text-emerald-600 uppercase tracking-wider">{t('reservas.tipo')}</th>
                                      <th className="px-4 py-2 text-left text-xs font-medium text-emerald-600 uppercase tracking-wider">{t('reservas.cantidadPorPaquete')}</th>
                                      <th className="px-4 py-2 text-left text-xs font-medium text-emerald-600 uppercase tracking-wider">{t('reservas.total')}</th>
                                    </tr>
                                  </thead>
                                  <tbody className="bg-white divide-y divide-emerald-200">
                                    {paquete.desglose_pasajes.map((desglose, idx) => {
                                      ...
                                      return (
                                        <tr key={idx}>
                                          <td className="px-4 py-2 whitespace-nowrap font-medium text-gray-900">{desglose.tipo}</td>
                                          <td className="px-4 py-2 whitespace-nowrap">{desglose.cantidad}</td>
                                          <td className="px-4 py-2 whitespace-nowrap font-bold text-emerald-600">{totalTipo}</td>
                                        </tr>
                                      );
                                    })}
                                  </tbody>
                                ...
                                <div className="bg-emerald-100 rounded-lg p-4 mt-4">
                                  <div className="text-sm text-emerald-800">
                                    <div className="font-bold mb-2 flex items-center">
                                      ...
                                      {t('reservas.totalEn')} {paquete.cantidad} {t('reservas.paquetes')}:
                                    </div>
                                    <div className="border-t border-emerald-200 pt-2 mt-2">
                                      <div className="flex justify-between font-bold text-emerald-900">
              ...
              {/* Información adicional - MEJORADA CON RESPONSIVE */}
              {(reserva.instancia?.punto_encuentro || reserva.instancia?.instrucciones || reserva.notas) && (
                <div className="bg-white rounded-2xl shadow-xl p-8 border border-blue-100">
                  <h3 className="text-2xl font-bold text-gray-900 mb-6 flex items-center">
                    <svg className="h-6 w-6 text-blue-600 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {/* Panel lateral - MEJORADO CON RESPONSIVE */}
            <motion.div
              className="space-y-8"
              initial={{ opacity: 0, x: 20 }}
              animate={{ opacity: 1, x: 0 }}
            ...
            >
                  ...
                  <div className="space-y-3">
                    ...
                    {reserva.cliente.correo && (
                      <div>
                        ...
                        <div className="text-gray-900">{reserva.cliente.correo}</div>
                      </div>
                    )}
                    {reserva.cliente.numero_celular && (
                      <div>
                        <div className="text-sm text-gray-500">{t('reservas.telefono')}</div>
                        <div className="text-gray-900">{reserva.cliente.numero_celular}</div>
                      </div>
                    )}
                    {reserva.cliente.tipo_documento && reserva.cliente.numero_documento && (
                      <div>
              ...
              <div className="bg-white rounded-2xl shadow-xl p-6 border border-blue-100">
                ...
                <div className="space-y-3">
                  ...
                  {reserva.fecha_cancelacion && (
                    <div>
                      <div className="text-sm text-gray-500">{t('reservas.fechaCancelacion')}</div>
                      <div className="text-red-600">{formatearFecha(reserva.fecha_cancelacion, 'corto')}</div>
                    </div>
                  )}
                </div>
              ...
              {/* Acciones - MEJORADAS CON RESPONSIVE */}
              <div className="bg-white rounded-2xl shadow-xl p-6 border border-blue-100">
