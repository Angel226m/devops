  ...
  const crearPreferenciaReal = useCallback(async () => {
    try {
      ...
      if (datosReserva.reservaId) {
        ...
        // 🔧 PROCESAR PASAJES INDIVIDUALES CORREGIDO
        let pasajes: Pasaje[] = [];
        ...
        if (datosReserva.cantidadesPasajes && Array.isArray(datosReserva.cantidadesPasajes)) {
          ...
          logger.info('PREFERENCE_CREATION', 'Pasajes individuales procesados', {
            cantidadTipos: pasajes.length,
            totalPasajeros: pasajes.reduce((sum, p) => sum + p.cantidad, 0)
          });
        } else if (datosReserva.seleccionPasajes) {
          pasajes = Object.entries(datosReserva.seleccionPasajes)
            .filter(([_, cantidadValue]) => {
          ...
          logger.info('PREFERENCE_CREATION', 'Pasajes desde seleccionPasajes', {
            cantidadTipos: pasajes.length,
            totalPasajeros: pasajes.reduce((sum, p) => sum + p.cantidad, 0)
          });
        }
        
        // 🔧 PROCESAR PAQUETES CORREGIDO
        let paquetes: Paquete[] = [];
        
        if (datosReserva.paquetes && Array.isArray(datosReserva.paquetes)) {
          paquetes = datosReserva.paquetes
            .filter((p: PaqueteDatosReserva) => p.cantidadPaquetes > 0)
            .map((p: PaqueteDatosReserva) => ({
              id_paquete: p.idPaquetePasajes,
          ...
          logger.info('PREFERENCE_CREATION', 'Paquetes procesados', {
            cantidadTiposPaquetes: paquetes.length,
            cantidadTotalPaquetes: paquetes.reduce((sum, p) => sum + p.cantidad, 0)
          });
        } else if (datosReserva.seleccionPaquetes) {
          paquetes = Object.entries(datosReserva.seleccionPaquetes)
            .filter(([_, cantidadValue]) => {
            ...
            .map(([paqueteId, cantidadValue]) => ({
              ...
              precio: 0 // Se calculará en el backend
            }));
            
          logger.info('PREFERENCE_CREATION', 'Paquetes desde seleccionPaquetes', {
            cantidadTiposPaquetes: paquetes.length,
            cantidadTotalPaquetes: paquetes.reduce((sum, p) => sum + p.cantidad, 0)
          });
        }
        ...
        logger.info('PREFERENCE_CREATION', 'Validación exitosa', {
          tienePasajes: pasajes.length > 0,
          tienePaquetes: paquetes.length > 0,
          totalPasajeros: datosReserva.totalPasajeros,
