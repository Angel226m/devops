...
// Definir el tipo OpcionHorario para hacer compatibles los datos
...
const PaginaProcesoPago = () => {
  ...
  const crearPreferenciaReal = useCallback(async () => {
    try {
      ...
      if (datosReserva.reservaId) {
        ...
        // Adaptar seleccionPaquetes al nuevo formato
        if (datosReserva.seleccionPaquetes) {
          paquetes = Object.entries(datosReserva.seleccionPaquetes)
            .filter(([_, cantidad]) => Number(cantidad) > 0)
            .map(([paqueteId, cantidad]) => ({
              id_paquete: parseInt(paqueteId),
        ...
        // Obtener el formato de horario correcto
        let horarioTexto = "";
        if (datosReserva.horarioSeleccionado && typeof datosReserva.horarioSeleccionado === 'object') {
          // Si tenemos el objeto OpcionHorario completo
          horarioTexto = datosReserva.horarioSeleccionado.texto;
        } else if (datosReserva.horario) {
          // Si tenemos solo el texto del horario
          horarioTexto = datosReserva.horario;
        }
        
        // Obtener ID de instancia correcto
        let idInstancia = 0;
        if (datosReserva.horarioSeleccionado && datosReserva.horarioSeleccionado.id) {
          // Si tenemos el objeto OpcionHorario completo
        ...
        // Marcar esta reserva como "en proceso" antes de enviar la solicitud
        const reservaTemporalId = `reserva_temp_${idInstancia}_${Date.now()}`;
        localStorage.setItem(cacheKey, JSON.stringify({
          temporalId: reservaTemporalId,
          timestamp: ahora,
          sessionId: sessionId
  ...
  const obtenerHorarioTexto = () => {
    if (datosReserva.horarioSeleccionado && typeof datosReserva.horarioSeleccionado === 'object') {
      return datosReserva.horarioSeleccionado.texto;
    } else if (datosReserva.horario) {
      return datosReserva.horario;
    }
                  ...
                  <div className="flex-1">
                    ...
                    <div className="mt-2 text-sm text-gray-600">
                      ...
                      <div className="flex items-center mt-1">
                        ...
                        <span className="font-medium">{obtenerHorarioTexto()}</span>
                      </div>
                    </div>
                  </div>
                  <div className="bg-gradient-to-r from-blue-50 to-cyan-50 text-blue-800 px-3 py-1.5 rounded-lg text-sm font-medium border border-blue-200">
