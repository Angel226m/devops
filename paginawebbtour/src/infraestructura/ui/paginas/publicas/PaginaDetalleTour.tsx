...
// Interfaz para los datos del tour
interface TourData {
  id: number;
  nombre: string;
  descripcion: string;
...
// Interfaz para los recursos cargados
interface RecursosCargados {
  tipoTour: boolean;
  tours: boolean;
  galeria: boolean;
...
// Función para formatear la hora
const formatearHora = (horaString: string | undefined): string => {
  if (!horaString) return '';
...
const PaginaDetalleTour = () => {
  ...
  const dispatch = useDispatch<AppDispatch>();
  const navigate = useNavigate();
  const [tabActiva, setTabActiva] = useState<'descripcion' | 'detalles' | 'horarios' | 'recomendaciones'>('descripcion');
  ...
  // Estados de Redux
  const { tipoTourActual, cargando: cargandoTipoTour, error: errorTipoTour } = useSelector((state: RootState) => state.tipoTour);
  const { toursDisponibles, cargando: cargandoTours, error: errorTours } = useSelector((state: RootState) => state.tourProgramado);
  const { galeriasTour, cargando: cargandoGaleria } = useSelector((state: RootState) => state.galeriaTour);
  ...
  const noIncluye = [
    'Impuesto portuario y SERNAMP (S/.16 por persona, pago en el muelle).',
    t('tour.noIncluye.comidas'),
    t('tour.noIncluye.propinas'),
  ];
  ...
  useEffect(() => {
    ...
    const idTipoTour = parseInt(idTour);
    if (isNaN(idTipoTour)) {
      console.error('ID de tour inválido:', idTour);
      return;
    }
    ...
    const cargarRecursos = async () => {
      try {
        if (!recursosCargados.current.tipoTour || (tipoTourActual && tipoTourActual.id_tipo_tour !== idTipoTour)) {
          await dispatch(obtenerTipoTourPorId(idTipoTour)).unwrap();
          recursosCargados.current.tipoTour = true;
        }

        if (!recursosCargados.current.tours) {
          await dispatch(listarToursDisponiblesSinDuplicados()).unwrap();
          recursosCargados.current.tours = true;
        }

        if (!recursosCargados.current.galeria) {
          await dispatch(listarGaleriasTourPorTipoTour(idTipoTour)).unwrap();
          recursosCargados.current.galeria = true;
        }

        if (!recursosCargados.current.tiposPasaje) {
          await dispatch(listarTiposPasajePorTipoTour(idTipoTour)).unwrap();
          recursosCargados.current.tiposPasaje = true;
        }

        if (!recursosCargados.current.paquetesPasajes) {
          await dispatch(listarPaquetesPasajesPorTipoTour(idTipoTour)).unwrap();
          recursosCargados.current.paquetesPasajes = true;
        }
      } catch (error) {
    ...
    cargarRecursos();
  }, [dispatch, idTour, tipoTourActual]);

  // Reiniciar referencias si cambia el ID del tour
  useEffect(() => {
    if (idTour && tipoTourActual && parseInt(idTour) !== tipoTourActual.id_tipo_tour) {
      recursosCargados.current = {
        tipoTour: false,
  ...
  const horariosUnicos = Array.from(
    new Set(
      toursDisponiblesParaEsteTipo.map((tour) => `${formatearHora(tour.hora_inicio)} - ${formatearHora(tour.hora_fin)}`)
    )
  );
  ...
  // Mostrar error si no se cargaron los datos
  if (errorTipoTour || !tipoTourActual) {
    return (
      <Seccion className="py-16 bg-gradient-to-b from-blue-50 via-sky-50 to-cyan-50">
        <div className="bg-white rounded-xl p-8 text-center shadow-lg max-w-2xl mx-auto border border-sky-100">
          ...
          <h2 className="text-2xl font-bold text-blue-800 mb-4">{errorTipoTour || t('tour.noEncontrado')}</h2>
          <p className="text-blue-700 mb-6">{t('tour.intentarDespues')}</p>
          <button
            onClick={() => navigate('/tours')}
            className="px-6 py-3 bg-teal-500 hover:bg-teal-600 text-white rounded-lg transition-colors duration-300 shadow-md"
          >
  ...
  const imagenTour =
    typeof tipoTourActual.url_imagen === 'object'
      ? tipoTourActual.url_imagen.Valid
        ? tipoTourActual.url_imagen.String
        : ''
      : tipoTourActual.url_imagen || '';

  // Calcular precio básico
  const precioBasico = Array.isArray(tiposPasaje) && tiposPasaje.length > 0 ? tiposPasaje[0].costo : 0;
  ...
  const tourData: TourData = {
    ...
    nombre: tipoTourActual.nombre,
    descripcion:
      typeof tipoTourActual.descripcion === 'string'
        ? tipoTourActual.descripcion
        : tipoTourActual.descripcion && tipoTourActual.descripcion.Valid
  ...
  return (
    <div className="bg-gradient-to-b from-blue-50 via-sky-50 to-cyan-50">
      <GaleriaTour idTipoTour={tipoTourActual.id_tipo_tour} nombreTour={tipoTourActual.nombre} imagenPrincipal={imagenTour} />
      <Seccion className="py-12">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2">
            <div className="bg-white/80 backdrop-blur-sm p-6 rounded-xl shadow-sm border border-sky-100">
              ...
              >
                <div className="flex items-center bg-amber-50/70 px-3 py-1 rounded-lg">
                  <div className="flex mr-1">{renderEstrellas(4.5)}</div>
                  <span className="text-black font-medium ml-1">4.5</span>
                </div>
                <div className="flex items-center text-black bg-sky-50/70 px-3 py-1 rounded-lg">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    className="h-5 w-5 mr-1 text-blue-700"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
                  ...
                  <span>
                    {tipoTourActual.duracion_minutos} {t('tour.minutos')}
                  </span>
                </div>
                <div className="flex items-center text-black bg-teal-50/70 px-3 py-1 rounded-lg">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    className="h-5 w-5 mr-1 text-teal-600"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
              ...
              >
                <div className="flex flex-wrap -mb-px">
                  {['descripcion', 'detalles', 'horarios', 'recomendaciones'].map((tab) => (
                    <button
                      key={tab}
                      className={`inline-block py-4 px-4 text-sm font-medium border-b-2 transition-colors duration-200 ${
                        tabActiva === tab ? 'border-teal-500 text-teal-600' : 'border-transparent text-blue-700 hover:text-blue-800'
                      }`}
              ...
              >
                {tabActiva === 'descripcion' && (
                  <div className="prose prose-lg max-w-none">
                    <p className="text-black">El tour incluye:</p>
                    <ul className="list-disc pl-6 mb-4">
                    ...
                    <p className="text-black font-medium">
                      Importante: La tarifa no incluye el impuesto portuario y el impuesto SERNAMP (S/.16 por persona, pago en el muelle).
                    </p>
                    {tourData.descripcion && tourData.descripcion.trim() !== '' && (
                      <div className="mt-4 pt-4 border-t border-gray-200">
                        <h3 className="text-lg font-semibold mb-2">Descripción adicional:</h3>
                ...
                {tabActiva === 'detalles' && (
                  <div className="grid md:grid-cols-2 gap-8">
                    <div className="bg-blue-50/70 p-4 rounded-lg">
                      <h3 className="text-xl font-semibold text-black mb-4">{t('tour.queIncluye')}</h3>
                      <ul className="space-y-2">
                        {incluye.map((item, index) => (
                          <li key={index} className="flex items-start">
                            <svg
                    ...
                    <div className="bg-red-50/70 p-4 rounded-lg">
                      <h3 className="text-xl font-semibold text-black mb-4">{t('tour.queNoIncluye')}</h3>
                      <ul className="space-y-2">
                ...
                {tabActiva === 'horarios' && (
                  <div>
                    <h3 className="text-xl font-semibold text-black mb-4">{t('tour.horariosDisponibles')}</h3>
                    {cargandoTours ? (
                      <div className="flex justify-center items-center py-8">
                        <Cargador tamanio="md" />
                      </div>
                      ...
                      <div className="bg-red-50 border-l-4 border-red-500 p-4 rounded-r-lg">
                        <p className="text-red-700">{errorTours}</p>
                      </div>
                    ) : toursDisponiblesParaEsteTipo.length === 0 ? (
                      <div className="bg-yellow-50 border-l-4 border-yellow-500 p-4 rounded-r-lg">
                        <p className="text-yellow-700">{t('tour.sinHorariosDisponibles')}</p>
                      </div>
                    ) : (
                      <div>
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mb-6">
                        ...
                        <div className="bg-blue-50 border-l-4 border-blue-500 p-4 rounded-r-lg mt-6">
                          ...
                          <ul className="space-y-2 text-blue-600">
                            <li className="flex items-start">
                              ...
                              <span>Los horarios mostrados corresponden a las próximas salidas disponibles.</span>
                            </li>
                            <li className="flex items-start">
                              <svg
                                xmlns="http://www.w3.org/2000/svg"
                              ...
                              <span>Se recomienda llegar al punto de embarque 30 minutos antes de la hora de salida.</span>
                            </li>
                            {toursDisponiblesParaEsteTipo.length > 0 && toursDisponiblesParaEsteTipo[0].vigencia_hasta && (
                              <li className="flex items-start">
                                <svg
                                ...
                                <span>Vigente hasta: {formatearFecha(toursDisponiblesParaEsteTipo[0].vigencia_hasta)}</span>
                              </li>
                            )}
                          </ul>
                        </div>
                ...
                {tabActiva === 'recomendaciones' && (
                  <div>
                    <h3 className="text-xl font-semibold text-black mb-4">{t('tour.recomendaciones')}</h3>
                    <div className="bg-blue-50/80 border-l-4 border-blue-500 p-4 rounded-r-lg mb-6">
                      <p className="text-blue-700">{t('tour.recomendacionesIntro')}</p>
                    </div>
                    <ul className="space-y-2 bg-cyan-50/70 p-4 rounded-lg">
      ...
      <Seccion className="py-12 bg-gradient-to-b from-sky-100 to-cyan-100">
        <motion.div
          ...
          className="text-center mb-12"
        >
          <h2 className="text-3xl font-bold text-blue-800 mb-4">{t('tour.toursRelacionados')}</h2>
          <p className="text-blue-700 max-w-2xl mx-auto">{t('tour.toursRelacionadosDescripcion')}</p>
        </motion.div>
        <ToursRelacionados idTourActual={tipoTourActual.id_tipo_tour} />
      </Seccion>
    </div>
