...
const SelectorPaquete = ({
  ...
  const obtenerDesglosePaquete = (): DesgloseItem[] => {
    if (!paquete.desglose_pasajes || !Array.isArray(paquete.desglose_pasajes)) {
      return paquete.cantidad_total ? [{ tipo: 'Pasajeros incluidos', cantidad: paquete.cantidad_total, id_tipo_pasaje: undefined }] : [];
    }
    return paquete.desglose_pasajes;
  };
            ...
            return (
              <div key={index} className="flex justify-between items-center bg-white p-2 rounded border border-emerald-100">
                <div className="flex items-center">
                  <span className="text-emerald-700 font-medium text-sm">{item.tipo}</span>
                  {tipoPasajeEncontrado && (
                    <span className="ml-2 text-xs text-emerald-600 bg-emerald-100 px-2 py-0.5 rounded-full">ID: {tipoPasajeEncontrado.id_tipo_pasaje}</span>
                  )}
                </div>
                <span className="text-emerald-900 font-bold text-sm">{item.cantidad}x</span>
              </div>
      ...
      <div className="relative">
        <div className="flex justify-between items-start mb-4">
          <div className="flex-1">
            ...
            <div className="flex flex-wrap gap-2 mb-3">
              ...
              <span className="inline-block bg-orange-100 text-orange-700 px-3 py-1 rounded-full text-sm font-semibold">🔥 ¡Oferta especial!</span>
              {desglosePaquete.length > 0 && (
                <span className="inline-block bg-blue-100 text-blue-700 px-3 py-1 rounded-full text-sm font-semibold">📋 {desglosePaquete.length} tipos</span>
              )}
            </div>
          </div>
          <div className="text-right ml-4">
            <div className="text-3xl font-bold text-emerald-600 mb-1">S/ {paquete.precio_total.toFixed(2)}</div>
            <div className="text-xs text-gray-500 line-through">S/ {(paquete.precio_total * 1.15).toFixed(2)}</div>
            <div className="text-xs text-emerald-600 font-medium">Ahorro: S/ {(paquete.precio_total * 0.15).toFixed(2)}</div>
          </div>
        </div>
        {paquete.descripcion && (
          <div className="bg-gray-50 rounded-lg p-3 mb-3 border border-gray-200">
        ...
        <div className="flex items-center justify-between mt-5">
          ...
          <div className="flex items-center space-x-3">
            ...
            <span className="w-16 text-center py-2 bg-teal-50 border border-teal-300 text-teal-900 font-bold rounded-lg text-lg">{cantidad}</span>
            <button
              type="button"
              onClick={() => setCantidad(Math.min(5, cantidad + 1))}
              className="w-10 h-10 bg-teal-100 hover:bg-teal-200 text-teal-700 rounded-lg border border-teal-300 transition-colors font-bold text-lg"
...
const FormularioReservacion = ({ tour }: FormularioReservacionProps) => {
  ...
  const irAPago = () => {
    ...
    Object.entries(pasajesDesglosadosDePaquetes).forEach(([id, cant]) => {
      ...
      pasajesTotalesCombinados[idNum] = (pasajesTotalesCombinados[idNum] || 0) + cant;
    });
        ...
        <div className="bg-white p-4 rounded-xl border border-teal-100 shadow-sm">
              ...
              <div className="space-y-1 text-xs">
                {Object.entries(seleccionPasajes)
                  ...
                  .map(([id, cant]) => {
                    const tipoPasaje = tiposPasajeDelTour.find((t) => t.id_tipo_pasaje === Number(id));
                    return (
                      <div key={id} className="flex justify-between bg-blue-50 p-2 rounded">
                        <span className="text-gray-600">{tipoPasaje?.nombre} ({cant}):</span>
                        <span className="text-blue-800 font-medium">S/ {((tipoPasaje?.costo || 0) * cant).toFixed(2)}</span>
                      </div>
                    );
                  })}
              </div>
              ...
              <div className="space-y-1 text-xs">
                {Object.entries(seleccionPaquetes)
                  ...
                  .map(([id, cant]) => {
                    const paquete = paquetesPasajesDelTour.find((p) => p.id_paquete === Number(id));
                    return (
                      <div key={id} className="bg-emerald-50 p-2 rounded">
                        <div className="flex justify-between mb-1">
                          <span className="text-gray-600 font-medium">{paquete?.nombre} ({cant}):</span>
                          <span className="text-emerald-800 font-medium">S/ {((paquete?.precio_total || 0) * cant).toFixed(2)}</span>
                        </div>
                        <div className="text-xs text-emerald-700 ml-2">
                          ↳ {paquete?.cantidad_total || 0} pasajeros × {cant} = {(paquete?.cantidad_total || 0) * cant} total
                        </div>
                      </div>
                    );
                  })}
          ...
          {Object.keys(pasajesDesglosados).length > 0 && (
            <div className="mt-4 pt-4 border-t border-gray-200">
              <div className="font-medium text-gray-700 text-sm mb-2 flex items-center">
                <FaBoxOpen className="w-3 h-3 mr-1" />
                🔄 Paquetes convertidos a pasajes:
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {Object.entries(pasajesDesglosados).map(([id, cant]) => {
                  const tipoPasaje = tiposPasajeDelTour.find((t) => t.id_tipo_pasaje === Number(id));
                  return (
                    <div key={id} className="bg-orange-50 p-2 rounded text-xs">
                      <div className="flex justify-between">
                        <span className="text-orange-700">{tipoPasaje?.nombre}:</span>
                        <span className="text-orange-800 font-bold">+{cant}</span>
                      </div>
                    </div>
                  );
                })}
      ...
      <div className="p-8 bg-gradient-to-b from-white to-teal-50">
            ...
            >
              ...
              >
                <div className="flex items-start">
                  <FaInfoCircle className="h-6 w-6 text-blue-500 mr-3 mt-0.5 flex-shrink-0" />
                  <div>
                    <div className="font-semibold text-blue-900 mb-2">🔄 Sistema de conversión automática</div>
                    <p className="text-sm text-blue-800 mb-2">Los paquetes se desglosan automáticamente en pasajes individuales.</p>
                  </div>
                </div>
              </motion.div>
              {paquetesPasajesDelTour.length > 0 && (
                <div className="space-y-6">
                  <div className="flex items-center justify-between mb-5">
                    <div className="flex items-center">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-emerald-600 font-medium">💰 ¡Ahorra comprando paquetes completos!</p>
                      </div>
                    </div>
                    <div className="text-right">
                      <div className="bg-emerald-100 text-emerald-800 px-3 py-1 rounded-full text-sm font-bold">
                  ...
                  <div className="text-center my-8">
                    <div className="relative">
                      ...
                      <div className="relative flex justify-center text-sm">
                        <span className="px-6 py-2 bg-teal-50 text-teal-700 rounded-full border border-teal-200 shadow-sm font-semibold">
                          ➕ Combina con pasajes individuales
                        </span>
                      </div>
                    </div>
                  </div>
                ...
                <div className="flex items-center justify-between mb-5">
                  <div className="flex items-center">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-blue-600 font-medium">🎯 Personaliza tu selección</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-sm font-bold">{tiposPasajeDelTour.length} tipos</div>
                  </div>
                </div>
                {tiposPasajeDelTour.length > 0 ? (
                  <div className="space-y-4">
                ...
                >
                  {cargando ? (
                    ...
                    <>
                      <FaCreditCard className="h-5 w-5 mr-2" />
                      {t('tour.continuarPago')} S/ {calcularTotal().toFixed(2)}
                      <span className="ml-2 text-xs opacity-90">({calcularTotalPasajeros()} {calcularTotalPasajeros() === 1 ? 'pasajero' : 'pasajeros'})</span>
                    </>
                  )}
                </motion.button>
              </div>
              ...
              >
                ...
                <div className="bg-gradient-to-br from-teal-50 to-cyan-50 p-6 rounded-2xl border border-teal-200 shadow-sm">
                  <p className="text-sm text-gray-700 mb-4 font-medium">💳 {t('tour.metodosPago')}:</p>
                  <div className="flex flex-wrap items-center gap-4 mb-6">
                    {[
                      { src: 'https://woocommerce.com/wp-content/uploads/2021/05/tw-mercado-pago-v2@2x.png', alt: 'Mercado Pago', height: 'h-9', badge: '🔒 Seguro', popular: true },
                      { src: 'https://images.freeimages.com/vme/images/7/1/715862/visa_logo_preview.jpg?h=350', alt: 'Visa', height: 'h-8', badge: '💳 Acepta', popular: true },
                      { src: 'https://images.freeimages.com/vme/images/9/9/99813/mastercard_logo_preview.jpg?h=350', alt: 'Mastercard', height: 'h-8', badge: '💳 Acepta', popular: true },
                      { src: 'https://logosenvector.com/logo/img/yape-bcp-4390.jpg', alt: 'Yape', height: 'h-8', badge: '📱 Móvil' },
                      { src: 'https://images.seeklogo.com/logo-png/38/1/plin-logo-png_seeklogo-386806.png', alt: 'Plin', height: 'h-8', badge: '📱 Móvil' },
                    ].map((img, index) => (
                      <motion.div
                        key={img.alt}
                        className="relative bg-white rounded-xl p-3 shadow-md border border-gray-100 group hover:shadow-lg transition-all"
                        ...
                        transition={{ delay: index * 0.1 }}
                      >
                        {img.popular && <div className="absolute -top-2 -right-2 bg-orange-500 text-white text-xs px-2 py-0.5 rounded-full font-bold">⭐</div>}
                        <img src={img.src} alt={img.alt} className={`${img.height} object-contain`} />
                        <div className="absolute -bottom-1 left-1/2 transform -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <span className="bg-teal-500 text-white text-xs px-2 py-1 rounded-full shadow-sm whitespace-nowrap">{img.badge}</span>
                        </div>
                      </motion.div>
                    ))}
                  </div>
                  ...
                  <div className="text-center">
                    <p className="text-xs text-teal-700 italic font-medium">🔒 {t('tour.pagoCifrado')} | ✅ Certificado PCI DSS | 🏆 Turismo Autorizado</p>
                  </div>
                </div>
              </motion.div>
            </motion.div>
        ...
        >
          <div className="flex items-start mb-4">
            <FaInfoCircle className="h-5 w-5 text-teal-500 mr-3 mt-0.5 flex-shrink-0" />
            <div>
              <div className="font-semibold text-gray-700 mb-3 text-base">📋 Términos y condiciones:</div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <p className="flex items-start text-sm">
                    <span className="text-teal-500 mr-2 mt-1">•</span>
                    <span><strong>Pago completo:</strong> Requerido al reservar</span>
                  </p>
                  <p className="flex items-start text-sm">
                    <span className="text-teal-500 mr-2 mt-1">•</span>
                    <span><strong>Reserva anticipada:</strong> Mínimo 24 horas</span>
                  </p>
                  <p className="flex items-start text-sm">
                    <span className="text-teal-500 mr-2 mt-1">•</span>
                    <span><strong>Desglose automático:</strong> Paquetes a pasajes</span>
                  </p>
                </div>
                <div className="space-y-2">
                  <p className="flex items-start text-sm">
                    <span className="text-teal-500 mr-2 mt-1">•</span>
                    <span><strong>Precios:</strong> Incluyen impuestos</span>
                  </p>
                  <p className="flex items-start text-sm">
                    <span className="text-teal-500 mr-2 mt-1">•</span>
                    <span><strong>Confirmación:</strong> Por email</span>
                  </p>
                  <p className="flex items-start text-sm">
                    <span className="text-teal-500 mr-2 mt-1">•</span>
                    <span><strong>Cancelación:</strong> Consulta términos</span>
                  </p>
                </div>
              </div>
            </div>
      ...
      <div className="bg-gradient-to-r from-teal-50 to-cyan-50 p-6 border-t border-teal-200">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm text-gray-700 mb-4">
          <div className="flex items-center justify-center sm:justify-start">
            ...
            <div>
              <span className="font-semibold text-teal-700">📞 Atención:</span>
              <br />
              <span className="text-teal-800 font-medium">+51 987 654 321</span>
            </div>
          </div>
        ...
        <div className="flex flex-wrap justify-center items-center gap-3 pt-4 border-t border-teal-200">
          {[
            ...
            { label: '🛡️ PCI DSS Compliant', color: 'purple' },
            { label: '🏆 Turismo Autorizado', color: 'orange' },
            { label: '⭐ 5 Estrellas', color: 'yellow' },
          ].map((cert, index) => (
            <motion.div
              key={index}
              className={`bg-${cert.color}-100 text-${cert.color}-800 px-3 py-1 rounded-full text-xs font-medium shadow-sm`}
...
// 🔧 COMPONENTE SELECTOR PAQUETE MEJORADO Y LIMPIO
const SelectorPaquete = ({
  paquete,
  cantidad,
  setCantidad,
  ...
  const mostrarDesglosePasajes = () => {
    ...
    return (
      <div className="mt-4 p-4 bg-emerald-50 rounded-lg border border-emerald-200">
        <div className="text-sm font-semibold text-emerald-800 mb-3 flex items-center">
          <FaBoxOpen className="w-4 h-4 mr-2" />
          Incluye por cada paquete:
        </div>
        <div className="space-y-2">
          {desglosePaquete.map((item: DesgloseItem, index: number) => {
            ...
            return (
              <div key={index} className="flex justify-between items-center bg-white p-3 rounded-md border border-emerald-100">
                <div className="flex items-center">
                  <span className="text-emerald-700 font-medium">{item.tipo}</span>
                  {tipoPasajeEncontrado && (
                    <span className="ml-3 text-xs text-emerald-600 bg-emerald-100 px-2 py-1 rounded-full">
                      ID: {tipoPasajeEncontrado.id_tipo_pasaje}
                    </span>
                  )}
                </div>
                <span className="text-emerald-900 font-bold">{item.cantidad}x</span>
              </div>
            );
          })}
        </div>
        <div className="mt-3 pt-3 border-t border-emerald-200 flex justify-between items-center">
          <span className="text-sm font-semibold text-emerald-800">Total de pasajeros:</span>
          <span className="text-emerald-900 font-bold text-lg">{paquete.cantidad_total}</span>
        </div>
      </div>
    );
  };
  ...
  return (
    <motion.div
      className={`relative p-6 bg-white border-2 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 ${
        cantidad > 0 
          ? 'border-emerald-400 shadow-emerald-100 bg-gradient-to-br from-emerald-50 to-teal-50' 
    ...
    >
      ...
      {cantidad > 0 && (
        <div className="absolute -top-3 -right-3 z-10">
          <div className="bg-emerald-500 text-white text-xs font-bold px-3 py-1 rounded-full shadow-lg flex items-center">
            <FaCheckCircle className="w-3 h-3 mr-1" />
            {cantidad} Seleccionado{cantidad > 1 ? 's' : ''}
          </div>
        </div>
      ...
      <div className="relative">
        ...
        <div className="flex justify-between items-start mb-4">
          <div className="flex-1">
            <h4 className="font-bold text-teal-900 text-xl mb-3 flex items-center">
              <FaUsers className="w-5 h-5 mr-2 text-emerald-600" />
              {paquete.nombre}
            </h4>
        ...
        <div className="flex items-center justify-between mt-5">
          ...
          <div className="flex items-center space-x-3">
            <button
              type="button"
              onClick={() => setCantidad(Math.max(0, cantidad - 1))}
              className="w-10 h-10 bg-teal-100 hover:bg-teal-200 text-teal-700 rounded-lg border border-teal-300 transition-colors font-bold text-lg focus:outline-none focus:ring-2 focus:ring-teal-500"
              aria-label="Reducir cantidad"
            >
              -
            </button>
...
const FormularioReservacion = ({ tour }: FormularioReservacionProps) => {
  ...
  // Handlers (mantener todos iguales)
  const handleFechaChange = (date: Date | null) => {
    if (!date) return;
    const ahora = new Date();
    const diferenciaDias = Math.floor((date.getTime() - ahora.getTime()) / (1000 * 60 * 60 * 24));
  ...
  // Desglose de paquetes en pasajes (mantener igual)
  const desglosarPaquetesEnPasajes = (): Record<number, number> => {
    const pasajesDesglosados: Record<number, number> = {};
    Object.entries(seleccionPaquetes).forEach(([idPaquete, cantidadPaquetes]) => {
      if (cantidadPaquetes > 0) {
  ...
  // 🔧 RESUMEN DETALLADO LIMPIO Y MEJORADO
  const ResumenDetallado = () => {
    const pasajerosIndividuales = Object.values(seleccionPasajes).reduce((total, cant) => total + cant, 0);
    const pasajerosPaquetes = Object.entries(seleccionPaquetes).reduce((total, [id, cant]) => {
      const paquete = paquetesPasajesDelTour.find((p) => p.id_paquete === Number(id));
    ...
    return (
      <motion.div
        ...
        transition={{ duration: 0.4 }}
      >
        <div className="font-bold text-teal-900 mb-6 flex items-center text-xl">
          <FaCalculator className="w-6 h-6 mr-3" />
          📊 Resumen detallado de la reserva
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          ...
          <div className="bg-white p-5 rounded-xl border border-teal-100 shadow-sm">
            <div className="font-semibold text-teal-800 mb-4 text-lg flex items-center">
            ...
            <div className="space-y-3 text-sm">
              <div className="flex justify-between items-center">
                <span className="text-gray-600">Individuales:</span>
                <span className="text-teal-800 font-medium text-lg">{pasajerosIndividuales}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-gray-600">En paquetes:</span>
                <span className="text-teal-800 font-medium text-lg">{pasajerosPaquetes}</span>
              </div>
              <div className="flex justify-between items-center pt-3 border-t border-teal-100">
                <span className="font-semibold text-teal-800 text-base">Total:</span>
                <span className="font-bold text-teal-900 text-2xl">{calcularTotalPasajeros()}</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white p-5 rounded-xl border border-teal-100 shadow-sm">
            ...
            <div className="space-y-3 text-sm">
              ...
              <div className="flex justify-between items-center pt-3 border-t border-teal-100">
                <span className="font-semibold text-teal-800 text-base">Total:</span>
                <span className="font-bold text-teal-900 text-xl">S/ {calcularTotal().toFixed(2)}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white p-5 rounded-xl border border-teal-100 shadow-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* PASAJES INDIVIDUALES */}
            <div>
              <div className="font-medium text-gray-700 text-base mb-3">Pasajes individuales:</div>
              <div className="space-y-2 text-sm">
                {Object.entries(seleccionPasajes)
                  .filter(([_, cant]) => cant > 0)
                  .map(([id, cant]) => {
                    const tipoPasaje = tiposPasajeDelTour.find((t) => t.id_tipo_pasaje === Number(id));
                    return (
                      <div key={id} className="flex justify-between bg-blue-50 p-3 rounded-lg">
                        <span className="text-gray-600 font-medium">{tipoPasaje?.nombre} ({cant}):</span>
                        <span className="text-blue-800 font-bold">S/ {((tipoPasaje?.costo || 0) * cant).toFixed(2)}</span>
                      </div>
                    );
                  })}
              </div>
            ...
            {/* PAQUETES */}
            <div>
              <div className="font-medium text-gray-700 text-base mb-3">Paquetes seleccionados:</div>
              <div className="space-y-2 text-sm">
                {Object.entries(seleccionPaquetes)
                  .filter(([_, cant]) => cant > 0)
                  .map(([id, cant]) => {
                    const paquete = paquetesPasajesDelTour.find((p) => p.id_paquete === Number(id));
                    return (
                      <div key={id} className="bg-emerald-50 p-3 rounded-lg">
                        <div className="flex justify-between mb-2">
                          <span className="text-gray-600 font-medium">{paquete?.nombre} ({cant}):</span>
                          <span className="text-emerald-800 font-bold">S/ {((paquete?.precio_total || 0) * cant).toFixed(2)}</span>
                        </div>
                        <div className="text-xs text-emerald-700 ml-2">
                          ↳ {paquete?.cantidad_total || 0} pasajeros × {cant} = {(paquete?.cantidad_total || 0) * cant} total
                        </div>
          ...
          {Object.keys(pasajesDesglosados).length > 0 && (
            <div className="mt-6 pt-6 border-t border-gray-200">
              <div className="font-medium text-gray-700 text-base mb-3 flex items-center">
                <FaBoxOpen className="w-4 h-4 mr-2" />
                🔄 Paquetes convertidos a pasajes individuales:
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {Object.entries(pasajesDesglosados).map(([id, cant]) => {
                  const tipoPasaje = tiposPasajeDelTour.find((t) => t.id_tipo_pasaje === Number(id));
                  return (
                    <div key={id} className="bg-orange-50 p-3 rounded-lg text-sm">
                      <div className="flex justify-between items-center">
                        <span className="text-orange-700 font-medium">{tipoPasaje?.nombre}:</span>
                        <span className="text-orange-800 font-bold">+{cant}</span>
                      </div>
                    </div>
                  );
  ...
  return (
    ...
    >
      ...
      <div className="p-8 bg-gradient-to-b from-white to-teal-50">
            ...
            >
              ...
              {paquetesPasajesDelTour.length > 0 && (
                <div className="space-y-6">
                  <div className="flex items-center justify-between mb-5">
                    <div className="flex items-center">
                      <div className="h-14 w-14 bg-gradient-to-br from-emerald-500 to-teal-600 text-white rounded-full flex items-center justify-center mr-4 shadow-lg">
                    ...
                    <div className="relative">
                      ...
                      <div className="relative flex justify-center text-sm">
                        <span className="px-6 py-2 bg-teal-50 text-teal-700 rounded-full border border-teal-200 shadow-sm font-semibold">
                          ...
                          {/* PASAJES INDIVIDUALES MEJORADOS - CONTINUACIÓN */}
              <div className="space-y-6">
                <div className="flex items-center justify-between mb-5">
                  <div className="flex items-center">
                    <div className="h-14 w-14 bg-gradient-to-br from-blue-500 to-cyan-600 text-white rounded-full flex items-center justify-center mr-4 shadow-lg">
                      <FaUserFriends className="h-8 w-8" />
                    </div>
                    <div>
                      <h4 className="text-xl font-bold text-teal-900">👤 {t('tour.pasajesIndividuales')}</h4>
                      <p className="text-sm text-blue-600 font-medium">🎯 Personaliza tu selección por tipo de pasajero</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="bg-blue-100 text-blue-800 px-4 py-2 rounded-full text-sm font-bold">
                      {tiposPasajeDelTour.length} tipos disponibles
                    </div>
                  </div>
                </div>
                ...
                {tiposPasajeDelTour.length > 0 ? (
                  ...
                  <div className="bg-teal-50 p-6 rounded-2xl text-center text-teal-800 border border-teal-200 shadow-sm">
                    <FaExclamationTriangle className="h-10 w-10 mx-auto mb-4 text-teal-600" />
                    <p className="font-medium text-lg">{t('tour.noHayTiposPasajeDisponibles')}</p>
                  </div>
                )}
              </div>
              ...
              <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mt-8">
                <motion.button
                  type="button"
                  onClick={() => setPasoActual(1)}
                  className="w-full sm:w-auto px-6 py-3 border-2 border-teal-300 text-teal-700 font-bold rounded-xl transition-all hover:bg-teal-50 shadow-md flex items-center justify-center focus:outline-none focus:ring-2 focus:ring-teal-500"
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.98 }}
                >
                  <FaTimes className="h-5 w-5 mr-2" />
                ...
                <motion.button
                  ...
                  onClick={irAPago}
                  disabled={!haySeleccion || cargando}
                  className={`w-full sm:w-auto px-8 py-4 bg-gradient-to-r from-teal-600 to-cyan-500 hover:from-teal-700 hover:to-cyan-600 text-white font-bold rounded-xl transition-all shadow-lg hover:shadow-xl flex items-center justify-center focus:outline-none focus:ring-2 focus:ring-teal-500 ${
                    !haySeleccion || cargando ? 'opacity-60 cursor-not-allowed' : ''
                  }`}
                  whileHover={{ scale: !haySeleccion || cargando ? 1 : 1.05 }}
                  whileTap={{ scale: !haySeleccion || cargando ? 1 : 0.95 }}
              ...
              <motion.div
                className="mt-10 pt-6 border-t border-teal-200"
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.4, delay: 0.2 }}
              >
                <div className="flex items-center mb-6">
                  <FaCreditCard className="h-7 w-7 text-teal-600 mr-3" />
                  <div className="font-bold text-teal-800 text-xl">🔐 {t('tour.pagoSeguro')}</div>
                </div>
                
                <div className="bg-gradient-to-br from-teal-50 to-cyan-50 p-6 rounded-2xl border border-teal-200 shadow-sm">
                  <p className="text-base text-gray-700 mb-6 font-medium">💳 {t('tour.metodosPago')}:</p>
                  ...
                  <div className="flex flex-wrap items-center gap-4 mb-6">
                    {[
                      ...
                      <motion.div
                        key={img.alt}
                        className="relative bg-white rounded-xl p-4 shadow-md border border-gray-100 group hover:shadow-lg transition-all"
                        whileHover={{ scale: 1.05, y: -2 }}
                        initial={{ opacity: 0, y: 10 }}
                        animate={{ opacity: 1, y: 0 }}
                        transition={{ delay: index * 0.1 }}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                    {[
                      { icon: '🔐', text: 'Encriptación SSL 256-bit', bgColor: 'bg-emerald-50', textColor: 'text-emerald-700', borderColor: 'border-emerald-100' },
                      { icon: '🛡️', text: 'Protección antifraude', bgColor: 'bg-blue-50', textColor: 'text-blue-700', borderColor: 'border-blue-100' },
                      { icon: '⚡', text: 'Procesamiento inmediato', bgColor: 'bg-purple-50', textColor: 'text-purple-700', borderColor: 'border-purple-100' }
                    ].map((feature, index) => (
                      <motion.div 
                        key={index}
                        className={`flex items-center p-4 ${feature.bgColor} rounded-lg shadow-sm border ${feature.borderColor}`}
                        initial={{ opacity: 0, x: -20 }}
                        animate={{ opacity: 1, x: 0 }}
                        transition={{ delay: 0.1 * index }}
                      >
                        <span className="text-2xl mr-3">{feature.icon}</span>
                        <span className={`text-sm ${feature.textColor} font-medium`}>{feature.text}</span>
                      </motion.div>
                    ))}
                  </div>
                  
                  <div className="text-center">
                    <p className="text-sm text-teal-700 italic font-medium">
                      🔒 {t('tour.pagoCifrado')} | ✅ Certificado PCI DSS | 🏆 Turismo Autorizado
                    </p>
                  </div>
                </div>
        ...
        <motion.div
          className="mt-12 text-sm text-gray-500 space-y-4 bg-gradient-to-r from-white via-gray-50 to-white p-8 rounded-2xl border border-teal-100 shadow-sm"
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.4 }}
        >
          <div className="flex items-start mb-6">
            <FaInfoCircle className="h-6 w-6 text-teal-500 mr-4 mt-1 flex-shrink-0" />
            <div>
              <div className="font-semibold text-gray-700 mb-4 text-lg">📋 Términos y condiciones importantes:</div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      {/* FOOTER MEJORADO CON INFORMACIÓN DE CONTACTO */}
      <div className="bg-gradient-to-r from-teal-50 to-cyan-50 p-8 border-t border-teal-200">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 text-sm text-gray-700 mb-6">
          <div className="flex items-center justify-center sm:justify-start">
            ...
            <div>
              <span className="font-semibold text-teal-700">📞 Atención al cliente:</span>
              <br />
              <span className="text-teal-800 font-medium text-base">+51 987 654 321</span>
            </div>
          </div>
          ...
          <div className="flex items-center justify-center sm:justify-start">
            ...
            <div>
              <span className="font-semibold text-teal-700">📧 Email:</span>
              <br />
              <span className="text-teal-800 font-medium text-base">reservas@tours-peru.com</span>
            </div>
          </div>
          ...
          <div className="flex items-center justify-center sm:justify-start">
            ...
            <div>
              <span className="font-semibold text-teal-700">⏰ Horario de atención:</span>
              <br />
              <span className="text-teal-800 font-medium text-base">Lun-Dom 8:00am-8:00pm</span>
            </div>
          </div>
        </div>
        
        {/* CERTIFICACIONES DE SEGURIDAD */}
        <div className="flex flex-wrap justify-center items-center gap-3 pt-6 border-t border-teal-200">
          {[
            { label: '🔒 SSL Seguro', bgColor: 'bg-emerald-100', textColor: 'text-emerald-800' },
            { label: '✅ MercadoPago Certificado', bgColor: 'bg-blue-100', textColor: 'text-blue-800' },
            { label: '🛡️ PCI DSS Compliant', bgColor: 'bg-purple-100', textColor: 'text-purple-800' },
            { label: '🏆 Turismo Autorizado', bgColor: 'bg-orange-100', textColor: 'text-orange-800' },
            { label: '⭐ 5 Estrellas en Servicio', bgColor: 'bg-yellow-100', textColor: 'text-yellow-800' }
          ].map((cert, index) => (
            <motion.div
              key={index}
              className={`${cert.bgColor} ${cert.textColor} px-4 py-2 rounded-full text-sm font-medium shadow-sm`}
              initial={{ opacity: 0, scale: 0.8 }}
              animate={{ opacity: 1, scale: 1 }}
              transition={{ delay: index * 0.1 }}
              whileHover={{ scale: 1.05 }}
