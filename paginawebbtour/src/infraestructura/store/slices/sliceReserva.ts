...
// 🔧 INTERFACE ACTUALIZADA CON reservaDetalle
export interface ReservaState {
  reservas: Reserva[];
  reservaActual: Reserva | null;
  reservaDetalle: Reserva | null; // ✅ AÑADIDA
  preferenciaPago: ReservaMercadoPagoResponse | null;
  cargando: boolean;
  error: string | null;
}

// 🔧 INITIAL STATE ACTUALIZADO
const initialState: ReservaState = {
  reservas: [],
  reservaActual: null,
  reservaDetalle: null, // ✅ AÑADIDA
  preferenciaPago: null,
  cargando: false,
  error: null
};
...
// Thunks existentes (mantener todos)
export const listarReservas = createAsyncThunk(
  "reserva/listar",
  async () => {
    return await repoReserva.listar();
...
// 🔧 NUEVO THUNK ESPECÍFICO PARA DETALLE
export const obtenerDetalleReservaPorId = createAsyncThunk(
  "reserva/obtenerDetallePorId",
  async (id: number) => {
    return await repoReserva.obtenerPorId(id);
  }
);
...
// 🔧 SLICE ACTUALIZADO CON CASOS PARA reservaDetalle
const reservaSlice = createSlice({
  name: "reserva",
  initialState,
  reducers: {
    ...
    // 🔧 NUEVO REDUCER
    limpiarReservaDetalle: (state) => {
      state.reservaDetalle = null;
    },
    limpiarError: (state) => {
      state.error = null;
    },
  ...
  extraReducers: (builder) => {
    builder
      ...
      // Obtener reserva por ID (para reservaActual)
      .addCase(obtenerReservaPorId.pending, (state) => {
        state.cargando = true;
        state.error = null;
      })
      ...
      // 🔧 NUEVOS CASOS PARA reservaDetalle
      .addCase(obtenerDetalleReservaPorId.pending, (state) => {
        state.cargando = true;
        state.error = null;
      })
      .addCase(obtenerDetalleReservaPorId.fulfilled, (state, action) => {
        state.cargando = false;
        state.reservaDetalle = action.payload;
      })
      .addCase(obtenerDetalleReservaPorId.rejected, (state, action) => {
        state.cargando = false;
        state.error = action.error.message || "Error al obtener detalle de la reserva";
      })
...
// 🔧 EXPORTAR EL NUEVO REDUCER
export const { 
  limpiarReservaActual, 
  limpiarReservaDetalle, 
  limpiarError, 
  limpiarPreferenciaPago 
