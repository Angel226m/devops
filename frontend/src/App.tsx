...
import { checkAuthStatus } from './infrastructure/store/slices/authSlice'; // Eliminé getAuthCookies
...
const ProtectedRoute: React.FC<{ 
  ...
  const { isAuthenticated, user, selectedSede, sessionChecked, isLoading } = useSelector((state: RootState) => state.auth);
  const [localCheckDone, setLocalCheckDone] = useState(false);
  
  // Verificación al montar el componente (sin cookies, directo a API)
  useEffect(() => {
    const verifyAuth = async () => {
      // Si no estamos autenticados, verificar estado directamente
      if (!isAuthenticated && !isLoading) {
        console.log("[ProtectedRoute] Sin sesión activa, verificando...");
        try {
  ...
  console.log(`[ProtectedRoute] Usuario autenticado como: ${user.rol}, Roles permitidos:`, allowedRoles);
  
  // Si es admin y requiere sede pero no tiene seleccionada, redirigir a selección
  if (requireSede && !selectedSede && user.rol === 'ADMIN') {
    console.log("[ProtectedRoute] Admin sin sede seleccionada, redirigiendo a selección");
    return <Navigate to={ROUTES.AUTH.SELECT_SEDE} replace />;
  }
  
  // Verificar rol permitido
  if (allowedRoles.length > 0 && !allowedRoles.includes(user.rol)) {
    console.warn(`[ProtectedRoute] Usuario con rol ${user.rol} intentando acceder a ruta para ${allowedRoles.join(', ')}`);
  ...
  console.log(`[ProtectedRoute] Acceso permitido a usuario ${user.rol}`);
  return children;
};
...
const App: React.FC = () => {
  ...
  // Verificar autenticación inicial solo una vez (sin cookies, directo a API)
  useEffect(() => {
    const checkInitialAuth = async () => {
      if (!auth.isAuthenticated && !auth.isLoading) {
        ...
        try {
          await dispatch(checkAuthStatus()).unwrap();
          console.log("[App] Verificación de sesión exitosa");
        } catch (error) {
          console.error("[App] Error verificando sesión inicial:", error);
        }
      }
    ...
    if (!initialCheckDone && !auth.sessionChecked) {
      checkInitialAuth();
    }
  }, [dispatch, auth.isAuthenticated, auth.isLoading, auth.sessionChecked, initialCheckDone]);
  
  // Mostrar loader mientras verificamos la sesión inicial
  if (!initialCheckDone && auth.isLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-blue-500"></div>
        <p className="ml-3 text-blue-600">Verificando sesión...</p>
      </div>
    );
  }
    ...
    <BrowserRouter>
      <Routes>
        ...
        <Route path="/vendedor" element={
          ...
          {/* NUEVAS RUTAS PARA EL NUEVO SISTEMA DE RESERVAS */}
          <Route path="reserva" element={<ReservaList />} /> {/* Nueva página principal usando tu componente existente */}
          <Route path="reserva/crear" element={<ReservaForm isEditing={false} />} /> {/* Reutilizando tu componente existente */}
          <Route path="reserva/editar/:id" element={<ReservaForm isEditing={true} />} /> {/* Reutilizando tu componente existente */}
          <Route path="reserva/ver/:id" element={<ReservaDetail />} /> {/* Reutilizando tu componente existente */}
...
import { BrowserRouter, Routes, Route, Navigate } from 'react-router-dom';
import { useDispatch, useSelector } from 'react-redux';
import { checkAuthStatus, getAuthCookies } from './infrastructure/store/slices/authSlice';
import { RootState, AppDispatch } from './infrastructure/store/index';
import { ROUTES } from './shared/constants/appRoutes';
...
const ProtectedRoute: React.FC<{ 
  ...
  // Verificación de cookies al montar el componente
  useEffect(() => {
    const verifyCookiesAndAuth = async () => {
      // Verificar si hay cookies de autenticación
      const cookieStatus = getAuthCookies();
      
      // Si hay cookies pero no estamos autenticados, verificar estado
      if (cookieStatus.hasAnyCookie && !isAuthenticated && !isLoading) {
        console.log("[ProtectedRoute] Cookies detectadas pero sin sesión activa, verificando...");
        try {
          await dispatch(checkAuthStatus()).unwrap();
        } catch (error) {
          console.error("[ProtectedRoute] Error verificando sesión:", error);
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const checkInitialAuth = async () => {
      ...
      if (cookieStatus.hasAnyCookie && !auth.isAuthenticated && !auth.isLoading) {
        console.log("[App] Cookies de autenticación detectadas, verificando sesión...");
        try {
          await dispatch(checkAuthStatus()).unwrap();
          console.log("[App] Verificación de sesión exitosa");
        } catch (error) {
      ...
      } else if (!cookieStatus.hasAnyCookie) {
        console.log("[App] No se encontraron cookies de autenticación");
      }
