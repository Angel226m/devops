...
import Button from '../components/Button';
import { FaShip, FaClock, FaUserFriends, FaMoneyBillWave, FaCalendarAlt, FaSearch, FaMapMarkerAlt, FaCalendarCheck, FaTicketAlt, FaBox, FaInfoCircle, FaSync, FaStar, FaTimes } from 'react-icons/fa';
import { format, parse, isValid, differenceInMinutes, addDays, parseISO, isPast } from 'date-fns';
import { es } from 'date-fns/locale';
import axios from '../../../infrastructure/api/axiosClient';
import { endpoints } from '../../../infrastructure/api/endpoints';
import { useNavigate } from 'react-router-dom';
...
const ToursDisponiblesPage: React.FC = () => {
  ...
  const getCurrentDate = () => {
    const now = new Date(2025, 8, 22); // Mes es 0-indexed, septiembre es 8
    return format(now, 'yyyy-MM-dd');
  };
    ...
    try {
      ...
      const fetchInstanciasTour = async () => {
        ...
        try {
          ...
          // Filtrar tours pasados si la fecha seleccionada es hoy
          const now = new Date();
          const filteredEnriquecidas = instanciasEnriquecidas.filter(instancia => {
            if (instancia.fecha_especifica_str === currentDate) {
              const tourStartTime = parse(instancia.hora_inicio_str || instancia.hora_inicio, instancia.hora_inicio_str ? 'HH:mm' : 'HH:mm:ss', new Date());
              const tourFullStart = new Date(instancia.fecha_especifica_str);
              tourFullStart.setHours(tourStartTime.getHours(), tourStartTime.getMinutes(), tourStartTime.getSeconds());

              if (isPast(tourFullStart)) {
                return false; // Excluir tours que ya han empezado
              }
            }
            return true;
          });

          setInstanciasTour(filteredEnriquecidas);
          setFilteredInstancias(filteredEnriquecidas);
          console.log("Datos de instancias enriquecidas cargados correctamente");
          return true;
        } catch (error) {
          console.error('Error al cargar instancias de tour:', error);
  ...
  const renderFechaCorta = (fecha: string): React.ReactNode => {
    try {
      ...
      const esHoy = fecha === currentDate;
      const esMañana = fecha === format(addDays(new Date(2025, 8, 22), 1), 'yyyy-MM-dd');
        ...
        <div className="bg-white rounded-2xl shadow-lg p-8 overflow-x-auto border border-gray-100">
          ...
          <div className="flex space-x-3">
            {nextDates.map((date, index) => {
              const esSeleccionado = selectedDate === date;
              const esHoy = date === currentDate;
              const esMañana = date === format(addDays(new Date(2025, 8, 22), 1), 'yyyy-MM-dd');
