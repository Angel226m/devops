...
import Button from '../components/Button';
import { FaShip, FaClock, FaUserFriends, FaMoneyBillWave, FaCalendarAlt, FaSearch, FaMapMarkerAlt, FaCalendarCheck, FaTicketAlt, FaBox, FaInfoCircle, FaSync, FaStar, FaTimes } from 'react-icons/fa';
import { format, parse, isValid, differenceInMinutes, addDays, parseISO, isAfter, addHours } from 'date-fns';
import { es } from 'date-fns/locale';
import axios from '../../../infrastructure/api/axiosClient';
import { endpoints } from '../../../infrastructure/api/endpoints';
import { useNavigate } from 'react-router-dom';
...
const MisReservasPage: React.FC = () => {
  ...
  const calculateStatus = (instancia: InstanciaTour): 'normal' | 'ask_departure' | 'blocked' => {
    try {
      const tourStart = parse(`${instancia.fecha_especifica} ${instancia.hora_inicio}`, 'yyyy-MM-dd HH:mm:ss', new Date());
      if (!isValid(tourStart)) return 'normal';
      const now = new Date();
      const threeHoursAfter = addHours(tourStart, 3);
      if (isAfter(now, threeHoursAfter)) {
        return 'blocked';
      } else if (isAfter(now, tourStart)) {
        return 'ask_departure';
      } else {
        return 'normal';
      }
    } catch (error) {
      console.error('Error calculando estado del tour:', error);
      return 'normal';
    }
  };
  ...
  useEffect(() => {
    ...
    const displayable = filtered.filter(instancia => calculateStatus(instancia) !== 'blocked');
    setFilteredInstancias(displayable);
  }, [searchTerm, instanciasTour]);

  const handleCreateReserva = (instancia: InstanciaTour) => {
    const status = calculateStatus(instancia);
    if (status === 'blocked') {
      alert('Este tour ya ha pasado más de 3 horas desde su hora de inicio y no está disponible.');
      return;
    }
    if (status === 'ask_departure') {
      const hasDeparted = window.confirm('La hora de inicio ha pasado, pero aún no han transcurrido 3 horas. ¿Ha salido el tour ya? (Posibles demoras)');
      if (hasDeparted) {
        alert('No se puede reservar porque el tour ya ha salido.');
        return;
      }
      // Si no ha salido, proceder (debido a demora)
    }
    // Proceder a reservar
    navigate(`/vendedor/reservas/nueva?instanciaId=${instancia.id_instancia}`);
  };
