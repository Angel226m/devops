...
import { fetchReservas } from '../../../infrastructure/store/slices/reservaSlice';
...
import { 
  ...
  FaFilePdf, FaFileExcel, FaChartBar
} from 'react-icons/fa';
...
const ReservaAdmin: React.FC = () => {
  ...
  const calcularEstadisticas = () => {
    if (!Array.isArray(reservas)) return { total: 0, confirmadas: 0, canceladas: 0, totalVentas: 0 };
    
    return reservas.reduce((stats: any, reserva: any) => {
      stats.total++;
      if (reserva.estado === 'CONFIRMADO') stats.confirmadas++;
      if (reserva.estado === 'CANCELADO') stats.canceladas++;
      stats.totalVentas += parseFloat(reserva.total_pagar || 0);
      return stats;
    }, { total: 0, confirmadas: 0, canceladas: 0, totalVentas: 0 });
  };
    ...
    reservas.filter((reserva: any) => {
      ...
      if (dateRange.start && dateRange.end) {
        try {
          ...
          matchesDateRange = fechaComparar >= startDate && fechaComparar <= endDate;
        } catch (e) {
      ...
      return matchesSearch && matchesEstado && matchesTour && matchesDateRange;
    }) : [];
  ...
  const displayReservas = sortedReservas.length > 0 ? sortedReservas : [
    { 
      ...
      documento_cliente: 'DNI 45678912',
    ...
    { 
      ...
      documento_cliente: 'DNI 23456789',
    ...
    { 
      id_reserva: 3, 
      nombre_cliente: 'Roberto Guzmán', 
      documento_cliente: 'CE 123456789',
      nombre_tour: 'Isla Blanca',
      fecha_tour: '18/06/2025',
      hora_inicio_tour: '11:00',
      hora_fin_tour: '14:00',
      estado: 'CANCELADO',
      total_pagar: 200.00,
      cantidad_pasajes: [
        { id_tipo_pasaje: 1, nombre_tipo: 'Adulto', cantidad: 4 },
      ],
      vendedor: 'Diana Pérez'
    }
  ];
  
  // Usar rutas disponibles
  const handleCreateReserva = () => {
    // Ya que ROUTES.ADMIN.RESERVA no existe, vamos a usar la ruta directa
    navigate('/admin/reservas/nueva');
  };
  ...
  const handleEditReserva = (reserva: any) => {
    const idReserva = reserva.id_reserva || 0;
    navigate(`/admin/reservas/editar/${idReserva}`);
  };
  ...
  const handleViewReports = () => {
    ...
    navigate('/admin/reservas/reportes');
  };
  ...
  const handleApplyFilters = () => {
    ...
    console.log("Aplicando filtros:", { searchTerm, selectedEstado, selectedTour, dateRange });
  };
    ...
    { 
      ...
      accessor: (reserva: any) => (
        <div className="text-sm">
          <p>{reserva.vendedor || 'No asignado'}</p>
        </div>
      )
    },
    { 
    ...
    <div className="flex items-center justify-between py-2 px-4 bg-gray-50 border-b">
      <div className="flex items-center space-x-2 overflow-x-auto pb-2">
        ...
        <button 
          ...
          className={`px-2 py-1 rounded text-sm ${sortField === 'total_pagar' ? 'bg-blue-100 text-blue-800' : 'bg-gray-100'}`}
        >
          Total {sortField === 'total_pagar' && (
            <FaSort className="inline ml-1 text-xs" />
          )}
        </button>
      </div>
  ...
  const currentDateTimeUTC = () => {
    return '2025-07-20 21:49:12'; // Para el ejemplo usamos la fecha proporcionada
  };
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <div>
          ...
          <p className="text-gray-600 text-sm mt-1">
            {selectedSede ? `Sede: ${selectedSede.nombre}` : 'Gestión centralizada de reservas'}
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button 
            ...
            className="flex items-center gap-2 bg-gradient-to-r from-purple-500 to-purple-700 hover:from-purple-600 hover:to-purple-800 text-white shadow-md transition-all"
            variant="primary" // Cambiado de "info" a "primary" para evitar errores
          >
            <FaChartBar /> Reportes
          </Button>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <Card className="bg-gradient-to-br from-blue-50 to-blue-100 border-blue-200">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <p className="text-sm text-blue-700">Total Reservas</p>
        ...
        <Card className="bg-gradient-to-br from-purple-50 to-purple-100 border-purple-200">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <p className="text-sm text-purple-700">Total Ventas</p>
              <p className="text-2xl font-bold text-purple-900">S/ {stats.totalVentas.toFixed(2)}</p>
            </div>
          </div>
        </Card>
      </div>
          ...
          <div className="flex flex-col md:flex-row md:items-start gap-4">
            <div className="flex-1">
              <div className="relative">
                <FormInput
                  placeholder="Buscar por cliente, documento o tour..."
            ...
            <div className="flex flex-col space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <Button 
                  variant="primary" // Cambiado de "info" a "primary"
                  className="flex items-center gap-2"
      ...
      <Card className="shadow-lg border border-gray-200">
        ...
        {displayReservas.length > 0 && (
          <div className="flex justify-between items-center mt-4 px-2 text-sm text-gray-600">
            <span>Mostrando {displayReservas.length} reservas de un total de {reservas?.length || displayReservas.length}</span>
