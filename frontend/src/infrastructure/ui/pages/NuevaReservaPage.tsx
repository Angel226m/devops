...
import Button from '../components/Button'; // Componente Button personalizado
import {
...
const NuevaReservaPage: React.FC = () => {
  ...
  const cargarInstanciaTour = useCallback(async () => {
    ...
    try {
      ...
      // Depuración: Log de los datos de fecha y hora
      console.log('Datos de instanciaTour:', {
        fecha_especifica: instancia.fecha_especifica,
        fecha_especifica_str: instancia.fecha_especifica_str,
        hora_inicio: instancia.hora_inicio,
        hora_inicio_str: instancia.hora_inicio_str,
        hora_fin: instancia.hora_fin,
        hora_fin_str: instancia.hora_fin_str,
      });
  ...
  const crearReserva = async () => {
    ...
    try {
      ...
      if (usarPaquete && paqueteSeleccionado) {
        ...
        reservaData.cantidad_pasajes = pasajesSeleccionados
  ...
  // Función optimizada para formatear hora
  const formatearHora = (horaStr?: string, horaISO?: string): string => {
    console.log('Formateando hora:', { horaStr, horaISO }); // Log para depuración
    if (horaStr && /^[0-9]{2}:[0-9]{2}$/.test(horaStr)) {
      try {
        const parsedHora = parse(horaStr, 'HH:mm', new Date());
        if (isValid(parsedHora)) {
          return format(parsedHora, 'hh:mm a', { locale: es });
        }
      } catch (error) {
        console.error(`Error al parsear hora_str: ${horaStr}`, error);
      }
    }

    if (horaISO) {
      try {
        const parsedHora = parseISO(horaISO);
        if (isValid(parsedHora)) {
          return format(parsedHora, 'hh:mm a', { locale: es });
        }
      } catch (error) {
        console.error(`Error al parsear hora_iso: ${horaISO}`, error);
      }
    }
  ...
  // Función optimizada para formatear fecha
  const formatearFecha = (fechaStr?: string, fechaISO?: string): string => {
    console.log('Formateando fecha:', { fechaStr, fechaISO }); // Log para depuración
    if (fechaStr && /^[0-9]{4}-[0-9]{2}-[0-9]{2}$/.test(fechaStr)) {
      try {
        const parsedFecha = parse(fechaStr, 'yyyy-MM-dd', new Date());
        if (isValid(parsedFecha)) {
          return format(parsedFecha, 'EEEE dd MMMM yyyy', { locale: es });
        }
      } catch (error) {
        console.error(`Error al parsear fecha_str: ${fechaStr}`, error);
      }
    }

    if (fechaISO) {
      try {
        const parsedFecha = parseISO(fechaISO);
        if (isValid(parsedFecha)) {
          return format(parsedFecha, 'EEEE dd MMMM yyyy', { locale: es });
        }
      } catch (error) {
        console.error(`Error al parsear fecha_iso: ${fechaISO}`, error);
      }
    }
          ...
          <div className="flex flex-col md:flex-row gap-6">
            <div className="w-full md:w-1/3">
              <div className="h-64 overflow-hidden rounded-lg bg-gray-200">
                {instanciaTour.tour_programado?.tipo_tour?.url_imagen && /^https?:\/\//.test(instanciaTour.tour_programado.tipo_tour.url_imagen) ? (
                  <img
                    src={instanciaTour.tour_programado.tipo_tour.url_imagen}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              <div>
                ...
                <select
                  ...
                  disabled={!!clienteEncontrado?.id_cliente}
                >
                  <option value="DNI">DNI</option>
                  <option value="CE">Carné de Extranjería</option>
                  <option value="Pasaporte">Pasaporte</option>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div className="border border-gray-200 rounded-lg p-4">
              ...
              <div className="space-y-3">
                ...
                <div>
                  <p className="py-1 text-sm text-gray-500">{clienteEncontrado?.tipo_documento}</p>
                  <p className="font-semibold">{clienteEncontrado?.numero_documento}</p>
                </div>
...
interface PaquetePasajes {
  ...
  descripcion: string;
  precio_total: number;
  cantidad_total: number; // Ahora obligatorio
  eliminado: boolean;
}
interface GaleriaTour {
  id: number;
...
interface PaqueteSeleccionado {
  id_paquete: number;
  nombre?: string;
  precio_total: number; // Ahora obligatorio
  cantidad_total: number; // Ahora obligatorio
  cantidad: number;
  subtotal: number; // Ahora obligatorio
}
interface ReservaData {
  id_vendedor?: number;
  id_cliente: number;
...
const METODOS_PAGO = ['Efectivo', 'Transferencia', 'Crédito', 'Yape', 'Plin'] as const;
...
const NuevaReservaPage: React.FC = () => {
  ...
  const [pasajesSeleccionados, setPasajesSeleccionados] = useState<PasajeCantidad[]>([]);
  const [paquetesSeleccionados, setPaquetesSeleccionados] = useState<PaqueteSeleccionado[]>([]);
  const [notasReserva, setNotasReserva] = useState('');
  ...
  // Funciones para manejar pasajes
  const actualizarCantidadPasaje = (id: number, cantidad: number) => {
    const totalPasajes = calcularTotalPasajes();
    const nuevaCantidad = Math.max(0, cantidad);
    if (totalPasajes - (pasajesSeleccionados.find(p => p.id_tipo_pasaje === id)?.cantidad || 0) + nuevaCantidad > (instanciaTour?.cupo_disponible || 0)) {
      setError('No hay suficientes cupos disponibles para los pasajes seleccionados.');
      return;
    }
    setError(null);
    setPasajesSeleccionados(prevState =>
      prevState.map(pasaje => {
        if (pasaje.id_tipo_pasaje === id) {
          return {
            ...
            subtotal: nuevaCantidad * (pasaje.costo || 0)
          };
        }
        return pasaje;
      })
  ...
  const seleccionarPaquete = (paquete: PaquetePasajes, cantidad: number = 1) => {
    ...
    if (totalPasajes + (paquete.cantidad_total * nuevaCantidad) > (instanciaTour?.cupo_disponible || 0)) {
      setError('No hay suficientes cupos disponibles para los paquetes seleccionados.');
      return;
    }
    setError(null);
    const existente = paquetesSeleccionados.find(p => p.id_paquete === paquete.id_paquete);
    if (existente) {
        ...
        paquetesSeleccionados.map(p => {
          if (p.id_paquete === paquete.id_paquete) {
            return {
              ...p,
              cantidad: nuevaCantidad,
              subtotal: nuevaCantidad * paquete.precio_total
            };
          }
          return p;
        })
      ...
      setPaquetesSeleccionados([
        ...paquetesSeleccionados,
        {
          ...
          cantidad_total: paquete.cantidad_total,
          cantidad: nuevaCantidad,
          subtotal: paquete.precio_total * nuevaCantidad
        }
      ]);
    }
  };

  const actualizarCantidadPaquete = (id: number, cantidad: number) => {
    const paquete = paquetesSeleccionados.find(p => p.id_paquete === id);
    if (!paquete) return;
    const totalPasajes = calcularTotalPasajes();
    const nuevaCantidad = Math.max(0, cantidad);
    if (totalPasajes - (paquete.cantidad * paquete.cantidad_total) + (nuevaCantidad * paquete.cantidad_total) > (instanciaTour?.cupo_disponible || 0)) {
      setError('No hay suficientes cupos disponibles para los paquetes seleccionados.');
      return;
    }
    setError(null);
    if (nuevaCantidad === 0) {
      setPaquetesSeleccionados(paquetesSeleccionados.filter(p => p.id_paquete !== id));
    } else {
      setPaquetesSeleccionados(
        paquetesSeleccionados.map(p => {
          if (p.id_paquete === id) {
            return {
              ...p,
              cantidad: nuevaCantidad,
              subtotal: nuevaCantidad * p.precio_total
            };
          }
          return p;
        })
  ...
  const calcularTotalPasajes = () => {
    const pasajesIndividuales = pasajesSeleccionados.reduce((sum, pasaje) => sum + pasaje.cantidad, 0);
    const pasajesPaquetes = paquetesSeleccionados.reduce((sum, paquete) => sum + paquete.cantidad_total * paquete.cantidad, 0);
    return pasajesIndividuales + pasajesPaquetes;
  };
  ...
  useEffect(() => {
    ...
    setTotalReserva(total);
  }, [pasajesSeleccionados, paquetesSeleccionados]);

  // Validar que se pueda avanzar al siguiente paso
  const validarPasoActual = (): boolean => {
    switch (currentStep) {
      case 1:
        return !!instanciaTour && instanciaTour.cupo_disponible > 0;
      case 2:
        return !!clienteEncontrado && !!clienteEncontrado.id_cliente;
      case 3:
        return (pasajesSeleccionados.some(p => p.cantidad > 0) || paquetesSeleccionados.length > 0) &&
               calcularTotalPasajes() <= (instanciaTour?.cupo_disponible || 0) &&
               METODOS_PAGO.includes(metodoPago);
      case 4:
        return totalReserva > 0 && METODOS_PAGO.includes(metodoPago);
      default:
        return false;
    }
  };
  ...
  const avanzarPaso = () => {
    if (validarPasoActual()) {
      ...
      setError('Por favor, complete todos los campos requeridos, incluyendo el método de pago.');
    }
  };
  ...
  // Función para crear reserva
  const crearReserva = async () => {
    if (!instanciaTour || !clienteEncontrado?.id_cliente || !METODOS_PAGO.includes(metodoPago)) {
      setError('Información incompleta para crear la reserva o método de pago inválido');
      return;
    }
    ...
    try {
      ...
      const reservaData: ReservaData = {
        ...
        cantidad_pasajes: pasajesSeleccionados
          .filter(p => p.cantidad > 0)
          .map(p => ({
            id_tipo_pasaje: p.id_tipo_pasaje,
            cantidad: p.cantidad
      ...
      if (paquetesSeleccionados.length > 0) {
        // Asumimos que el backend puede manejar el primer paquete; ajustar si soporta múltiples
        reservaData.id_paquete = paquetesSeleccionados[0].id_paquete;
      }
  ...
  const renderPaso1 = () => (
    <div className="bg-white rounded-xl shadow-lg p-6 mb-6">
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-8">
          <p className="text-lg font-semibold text-gray-600">Cargando información del tour...</p>
        </div>
      ) : error ? (
        <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg">
          <p className="font-medium">{error}</p>
  ...
  // Componente para el paso 3: Selección de pasajes y paquetes
  const renderPaso3 = () => (
    <div className="bg-white rounded-xl shadow-lg p-6 mb-6">
      <h2 className="text-2xl font-bold text-gray-800 mb-4">Selección de Pasajes y Paquetes</h2>

      <div className="mb-6 bg-blue-50 border border-blue-200 text-blue-700 p-4 rounded-lg">
        <p>Seleccione los pasajes individuales y/o paquetes para la reserva. Total de cupos disponibles: {instanciaTour?.cupo_disponible || 0}.</p>
      </div>
          ...
          <div className="space-y-4">
            {instanciaTour.tour_programado.paquetes_pasajes.map(paquete => (
              <div
                key={paquete.id_paquete}
                className={`border rounded-lg p-4 cursor-pointer transition-all ${
                  paquetesSeleccionados.find(p => p.id_paquete === paquete.id_paquete)
                    ? 'border-green-500 bg-green-50'
                    : 'border-gray-200 hover:border-blue-300 hover:bg-blue-50'
                }`}
                onClick={() => seleccionarPaquete(paquete)}
                  ...
                  {paquetesSeleccionados.find(p => p.id_paquete === paquete.id_paquete) && (
                    <div className="flex items-center">
                      <Button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          actualizarCantidadPaquete(paquete.id_paquete, paquetesSeleccionados.find(p => p.id_paquete === paquete.id_paquete)!.cantidad - 1);
                        }}
                        disabled={paquetesSeleccionados.find(p => p.id_paquete === paquete.id_paquete)!.cantidad <= 0}
                      >
                        -
                      </Button>

                      <span className="bg-white px-3 py-1 border-t border-b">
                        {paquetesSeleccionados.find(p => p.id_paquete === paquete.id_paquete)!.cantidad}
                      </span>
      ...
      <div className="mt-6 p-4 bg-green-50 border border-green-200 rounded-lg">
        <div className="space-y-2">
          <div className="flex justify-between items-center">
            <span className="font-medium text-gray-700">Total Pasajes:</span>
            <span className="font-bold text-gray-800">{calcularTotalPasajes()}</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="font-medium text-gray-700">Método de Pago:</span>
            <span className="font-bold text-gray-800">{metodoPago}</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="font-medium text-gray-700">Total a Pagar:</span>
            <span className="font-bold text-xl text-green-600">S/ {totalReserva.toFixed(2)}</span>
          </div>
        </div>
      </div>
      ...
      {!reservaCreada ? (
        <>
          ...
          <div className="border border-gray-200 rounded-lg p-4 mb-6">
            <h3 className="font-bold text-lg text-gray-800 mb-3 flex items-center">
              <FaTicketAlt className="mr-2 text-purple-500" />
              Detalle de Pasajes y Paquetes
            </h3>

            {paquetesSeleccionados.length > 0 && (
              <div className="mb-4">
                <h4 className="font-semibold text-gray-700 mb-2">Paquetes Seleccionados</h4>
                <div className="bg-gray-100 p-3 rounded-lg mb-3 grid grid-cols-12 gap-4 font-bold">
                  <div className="col-span-8">Paquete</div>
                  <div className="col-span-2 text-center">Cantidad</div>
                  <div className="col-span-2 text-right">Subtotal</div>
                ...
                {paquetesSeleccionados.map(paquete => (
                  <div key={paquete.id_paquete} className="p-3 grid grid-cols-12 gap-4 border-b border-gray-200">
                    <div className="col-span-8">
                      <p className="font-semibold">{paquete.nombre}</p>
                      <p className="text-sm text-gray-500">Incluye {paquete.cantidad_total} pasajes</p>
                    </div>

                    <div className="col-span-2 text-center">{paquete.cantidad}</div>
            ...
            {pasajesSeleccionados.some(p => p.cantidad > 0) && (
              <div className="mb-4">
                <h4 className="font-semibold text-gray-700 mb-2">Pasajes Individuales</h4>
                <div className="bg-gray-100 p-3 rounded-lg mb-3 grid grid-cols-12 gap-4 font-bold">
                  <div className="col-span-8">Tipo de Pasaje</div>
                  <div className="col-span-2 text-center">Cantidad</div>
                  <div className="col-span-2 text-right">Subtotal</div>
            ...
            <div className="mt-4">
              ...
              <p className="font-semibold text-gray-800">{metodoPago}</p>
            </div>
        ...
        <div className="text-center py-8">
          ...
          <div className="bg-green-50 border border-green-200 rounded-lg p-6 max-w-md mx-auto mb-6">
            <p className="text-gray-700 mb-2">Número de Reserva: <span className="font-bold">#{reservaId}</span></p>
            <p className="text-gray-700 mb-2">Total: <span className="font-bold">S/ {totalReserva.toFixed(2)}</span></p>
            <p className="text-gray-700 mb-2">Método de Pago: <span className="font-bold">{metodoPago}</span></p>
            <p className="text-gray-700">Estado: <span className="font-bold text-green-600">RESERVADO</span></p>
          </div>
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 mb-6">
          <div className="flex flex-wrap justify-between">
            ...
            <div className={`step flex-1 flex flex-col items-center ${currentStep >= 3 ? 'text-blue-600' : 'text-gray-400'}`}>
              ...
              <span className="text-sm text-center">Pasajes y Paquetes</span>
            </div>
