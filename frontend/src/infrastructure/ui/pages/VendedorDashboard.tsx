...
const VendedorDashboard: React.FC = () => {
  ...
  useEffect(() => {
    const loadDashboardData = async () => {
      ...
      try {
        ...
        // Obtener datos de reservas (últimos 30 días)
        const hoy = new Date();
        ...
        const [reservasResponse, instanciasResponse, pagosResponse] = await Promise.all([
          ...
          // Obtener instancias de tour disponibles (para próximas salidas)
        ...
        if (instanciasResponse.data && instanciasResponse.data.data) {
          ...
          const salidasFiltradas = instanciasResponse.data.data.filter(
            (inst: InstanciaTour) => inst.fecha_especifica === hoyStr && inst.estado !== 'CANCELADO'
          );
        ...
        // Procesar datos para estadísticas diarias
        processDataForStats(reservasResponse.data.data, pagosResponse.data.data);
  ...
  const processDataForStats = (reservasData: Reserva[], pagosData: Pago[]) => {
    ...
    const metodoPagoArray: MetodoPagoStats[] = Array.from(metodoPagoMap.entries())
      .map(([nombre, valor], index) => ({
        nombre,
        valor: Math.round((valor / pagosData.reduce((sum, p) => p.estado === 'PROCESADO' ? sum + p.monto : sum, 0)) * 100),
    ...
    const tourArray: TourStats[] = Array.from(tourMap.entries())
      .map(([nombre, valor], index) => ({
        nombre,
        valor: Math.round((valor / reservasData.length) * 100),
    ...
    // Combinar datos de reservas y pagos para crear el resumen de ventas
    const reservasHoy = reservasData.filter(r => r.fecha_reserva.startsWith(hoyStr));
    
    reservasHoy.forEach(reserva => {
      const pagoAsociado = pagosData.find(p => p.id_reserva === reserva.id_reserva);
      ...
      ventasHoyArray.push({
        ...
        metodoPago: pagoAsociado?.metodo_pago || 'Pendiente',
  ...
  // Calcular totales
  const totalVentasHoy = ventasHoy.reduce((sum, venta) => sum + venta.monto, 0);
  const ventasCompletadasHoy = ventasHoy.filter(v => v.estado === 'completado' || v.estado === 'procesado');
  const totalVentasCompletadasHoy = ventasCompletadasHoy.reduce((sum, venta) => sum + venta.monto, 0);
  const totalReservasHoy = ventasHoy.length;
  const reservasPendientesHoy = ventasHoy.filter(v => v.estado === 'pendiente').length;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-lg shadow-md p-5 border-t-4 border-amber-500">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-gray-500 text-sm">Pasajeros hoy</p>
              <p className="text-2xl font-bold text-gray-800">{Math.round(totalReservasHoy * 2.5)}</p>
              <p className="text-xs text-gray-500 mt-1">
                <span className="font-medium">{Math.round(reservasPendientesHoy * 2.5)}</span> pendientes por llegar
              </p>
            </div>
            <div className="bg-amber-100 p-3 rounded-full">
              <FiUsers className="text-amber-500 text-xl" />
          ...
          <div className="mt-2 text-xs text-gray-500">
            <span className="font-medium">
              {Math.round((totalReservasHoy - reservasPendientesHoy) * 2.5)} ya embarcados
            </span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-lg shadow-md overflow-hidden">
          ...
          <div className="p-4">
                ...
                <tbody className="divide-y divide-gray-200">
                  {ventasHoy.length > 0 ? ventasHoy.map((venta) => (
                    <tr key={venta.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-3 py-2 whitespace-nowrap text-xs text-center">
                        <span className={`px-2 py-1 rounded-full ${
                          venta.estado === 'completado' || venta.estado === 'procesado' ? 'bg-green-100 text-green-800' : 
                          venta.estado === 'pendiente' ? 'bg-yellow-100 text-yellow-800' : 
                          'bg-red-100 text-red-800'
                        }`}>
                          {venta.estado === 'completado' || venta.estado === 'procesado' ? 'Completado' : 
                           venta.estado === 'pendiente' ? 'Pendiente' : 'Cancelado'}
                        </span>
                      </td>
                    </tr>
                    ...
                    <tr>
                      <td colSpan={4} className="px-3 py-4 text-center text-gray-500">No hay ventas registradas hoy</td>
                    </tr>
                  )}
                </tbody>
              </table>
            ...
            {ventasHoy.length > 0 && (
              <div className="mt-3 text-right">
                <Link to="/vendedor/reservas" className="text-sm text-blue-600 hover:text-blue-800 flex items-center justify-end w-full">
                  Ver todas las ventas <FiChevronRight className="ml-1" />
                </Link>
              </div>
            )}
        ...
        <div className="bg-white rounded-lg shadow-md overflow-hidden">
          ...
          <div className="p-4">
            ...
            {salidasHoy.length > 0 && (
              <div className="mt-3 text-right">
                <Link to="/vendedor/instancias-tour" className="text-sm text-blue-600 hover:text-blue-800 flex items-center justify-end w-full">
                  Ver todas las salidas <FiChevronRight className="ml-1" />
                </Link>
              </div>
            )}
