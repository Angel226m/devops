...
const PagosVendedorPage: React.FC = React.memo(() => {
  const dispatch = useDispatch<AppDispatch>();
  const navigate = useNavigate();
  const { user, selectedSede } = useSelector((state: RootState) => state.auth);
  ...
  // Caché para almacenar pagos temporalmente
  const CACHE_KEY = `pagos_vendedor_${selectedSede?.id_sede || 'no_sede'}`;
  
  // Cargar datos de ejemplo cuando hay un error
  const loadSampleData = useCallback(() => [
    { 
      id_pago: 1, id_reserva: 101, NombreCliente: 'Juan', ApellidosCliente: 'Pérez', 
      DocumentoCliente: '45678912', TourNombre: 'Islas Ballestas', TourFecha: '2025-06-09',
      fecha_pago: '2025-06-09', metodo_pago: 'EFECTIVO', canal_pago: 'LOCAL', monto: 150.00,
      estado: 'PROCESADO', comprobante: 'Boleta', numero_comprobante: 'B-001-123'
    },
    { 
      id_pago: 2, id_reserva: 102, NombreCliente: 'María', ApellidosCliente: 'López',
      DocumentoCliente: '12345678', TourNombre: 'Reserva de Paracas', TourFecha: '2025-06-08',
      fecha_pago: '2025-06-08', metodo_pago: 'TARJETA', canal_pago: 'WEB', monto: 100.00,
      estado: 'PROCESADO', comprobante: 'Factura', numero_comprobante: 'F-001-456'
    },
    { 
      id_pago: 3, id_reserva: 103, NombreCliente: 'Carlos', ApellidosCliente: 'Rodríguez',
      DocumentoCliente: '87654321', TourNombre: 'Islas Ballestas', TourFecha: '2025-06-07',
      fecha_pago: '2025-06-07', metodo_pago: 'TRANSFERENCIA', canal_pago: 'LOCAL', monto: 200.00,
      estado: 'PENDIENTE', comprobante: 'Pendiente', numero_comprobante: null
    }
  ], []);

  // Cargar pagos desde la API o caché
  useEffect(() => {
    const fetchPagos = async () => {
      // Verificar caché
      const cachedData = sessionStorage.getItem(CACHE_KEY);
      if (cachedData) {
        setPagos(JSON.parse(cachedData));
        setLoading(false);
        return;
      }
      ...
      try {
        ...
        const reservas = reservasResponse.data.data;
        // Obtener pagos en paralelo usando Promise.all
        const pagosPromises = reservas.map((reserva: any) =>
          axios.get(endpoints.pago.vendedorListByReserva(reserva.id_reserva))
            .then(pagosResponse => ({
              reserva,
        ...
        pagosResults.forEach(({ reserva, pagos }) => {
          const pagosAdaptados = pagos.map((pago: any) => adaptarPago({
            ...pago,
            NombreCliente: reserva.nombre_cliente || reserva.nombres,
            ApellidosCliente: reserva.apellidos,
            DocumentoCliente: reserva.documento_cliente || reserva.numero_documento,
          ...
          todosLosPagos = [...todosLosPagos, ...pagosAdaptados];
        });

        // Ordenar por fecha_pago (más recientes primero)
        todosLosPagos.sort((a, b) => new Date(b.fecha_pago).getTime() - new Date(a.fecha_pago).getTime());

        // Guardar en caché
        sessionStorage.setItem(CACHE_KEY, JSON.stringify(todosLosPagos));
        setPagos(todosLosPagos);
        ...
        setApiError(error.message || 'Error al cargar los pagos. Se muestran datos de ejemplo.');
        const sampleData = loadSampleData();
        sessionStorage.setItem(CACHE_KEY, JSON.stringify(sampleData));
        setPagos(sampleData);
      } finally {
        setLoading(false);
      }
    ...
    fetchPagos();
  }, [dispatch, selectedSede, loadSampleData, CACHE_KEY]);
  ...
  const handleViewPago = useCallback((pago: Pago) => {
    setSelectedPago(pago);
    setIsModalOpen(true);
  }, []);

  const handlePrintComprobante = useCallback((pago: Pago) => {
    console.log('Imprimir comprobante para pago:', pago);
  }, []);

  const handleGenerarComprobante = useCallback((pago: Pago) => {
    console.log('Generar comprobante para pago:', pago);
  }, []);

  const handleVerReserva = useCallback((idReserva: number) => {
    navigate(ROUTES.VENDEDOR.RESERVA.VER(idReserva.toString()));
  }, [navigate]);

  const handleSubmitPago = useCallback(async (e: React.FormEvent) => {
    e.preventDefault();
    setIsModalOpen(false);
  ...
  // Formatear moneda
  const formatMoneda = useCallback((valor: number) => new Intl.NumberFormat('es-PE', {
    style: 'currency',
    currency: 'PEN'
  }).format(valor), []);
  ...
  const getNombreCompleto = useCallback((pago: Pago) => {
    const nombre = pago.NombreCliente || '';
    const apellidos = pago.ApellidosCliente || '';
    return `${nombre} ${apellidos}`.trim();
  }, []);

  // Filtrado de pagos con memoización
  const filteredPagos = useMemo(() => {
    const searchTermLower = searchTerm.toLowerCase();
    return pagos.filter(pago => {
      const nombreCompleto = getNombreCompleto(pago).toLowerCase();
      const tourNombre = (pago.TourNombre || '').toLowerCase();
      return nombreCompleto.includes(searchTermLower) || tourNombre.includes(searchTermLower);
    });
  }, [pagos, searchTerm, getNombreCompleto]);

  // Totales con memoización
  ...
  const columns = useMemo(() => [
    { header: 'Cliente', accessor: (row: Pago) => getNombreCompleto(row) },
    { header: 'Tour', accessor: 'TourNombre' },
    { header: 'Fecha Tour', accessor: (row: Pago) => formatearFecha(row.TourFecha || '') },
    { header: 'Fecha Pago', accessor: (row: Pago) => formatearFecha(row.fecha_pago) },
    { header: 'Método', accessor: 'metodo_pago' },
    { header: 'Monto', accessor: (row: Pago) => formatMoneda(row.monto) },
    { 
      header: 'Estado', 
      accessor: (row: Pago) => (
        <span className={`px-2 py-1 rounded-full text-xs font-medium ${row.estado === 'PROCESADO' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
          {row.estado}
        </span>
      )
    },
    ...
    },
    {
      ...
      accessor: (row: Pago) => (
        <div className="flex space-x-2">
          <button onClick={() => handleViewPago(row)} className="p-1 text-blue-600 hover:text-blue-800" title="Ver detalles">
            <FaEye />
          </button>
          {row.estado === 'PROCESADO' && (
            <button onClick={() => handlePrintComprobante(row)} className="p-1 text-green-600 hover:text-green-800" title="Imprimir comprobante">
              <FaPrint />
            </button>
          )}
          {row.estado === 'PENDIENTE' && (
            <button onClick={() => handleGenerarComprobante(row)} className="p-1 text-yellow-600 hover:text-yellow-800" title="Generar comprobante">
              <FaFileInvoice />
            </button>
          )}
        </div>
  ...
  ], [handleViewPago, handlePrintComprobante, handleGenerarComprobante, formatMoneda, formatearFecha, getNombreCompleto]);
  ...
  return (
    <div className="py-6 px-4 space-y-6 bg-gradient-to-br from-blue-50 to-indigo-50">
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center">
          <h1 className="text-2xl font-bold text-gray-800">Gestión de Pagos</h1>
          <Button onClick={handleRegistrarPago} className="flex items-center gap-2" variant="success">
            <FaPlus /> Registrar Pago
          </Button>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm mt-6">
          {loading ? (
            ...
            <Table columns={columns} data={filteredPagos} />
          )}
        </div>
